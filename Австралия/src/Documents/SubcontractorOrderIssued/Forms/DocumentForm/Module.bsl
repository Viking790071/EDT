...
Procedure InventoryPick(Command)
	...
	SelectionParameters = DriveClient.GetSelectionParameters(ThisObject,
		"Inventory",
		NStr("en = 'Subcontractor order issued'; ru = 'Выданный заказ на переработку';pl = 'Wydane zamówienie wykonawcy';es_ES = 'Orden emitida del subcontratista';es_CO = 'Orden emitida del subcontratista';tr = 'Alt yüklenici siparişi düzenlendi';it = 'Ordine subfornitura emesso';de = 'Subunternehmerauftrag ausgestellt'"),
		False,
		False);
...
Procedure FillByBasis(Command)
	...
	ShowQueryBox(New NotifyDescription("FillByBasisEnd", ThisObject),
		NStr("en = 'Do you want to repopulate the data for this Subcontractor order issued?'; ru = 'Вы хотите перезаполнять данные для этого выданного заказа на переработку?';pl = 'Czy chcesz ponownie wypełnić dane dla tego wydanego zamówienia podwykonawcy?';es_ES = '¿Quiere rellenar los datos de esta orden emitida del subcontratista?';es_CO = '¿Quiere rellenar los datos de esta orden emitida del subcontratista?';tr = 'Bu ""Düzenlenen alt yüklenici siparişi"" için verileri yeniden doldurmak istiyor musunuz?';it = 'Ripopolare i dati per questo Ordine subfornitura emesso?';de = 'Möchten Sie die Daten für diesen ausgestellten Subunternehmerauftrag neu füllen?'"),
...
Procedure AccountingPriceMessage()
	CommonClientServer.MessageToUser(NStr("en = 'The price is required for the by-product. Fill in the Accounting price in
		...
		|ru = 'Для побочной продукции необходимо указать цену. Заполните учетную цену в
		| учетной политике или укажите цену в карточке побочной продукции.';
		|pl = 'Wymagana jest cena produktu ubocznego. Wypełnij Cenę ewidencyjną w
		| Polityce rachunkowości lub określ cenę na karcie Produkty uboczne.';
		|es_ES = 'Se requiere el precio del trozo y deterioro. Rellene el Precio contable en
		| la Política de Contabilidad o especifique el precio en la tarjeta del trozo y deterioro.';
		|es_CO = 'Se requiere el precio del trozo y deterioro. Rellene el Precio contable en
		| la Política de Contabilidad o especifique el precio en la tarjeta del trozo y deterioro.';
		|tr = 'Yan ürün için fiyat gerekli. Muhasebe politikasında
		| muhasebe fiyatını doldurun veya yan ürün kartında fiyatı girin.';
		|it = 'Il prezzo è richiesto per il sottoprodotto. Compilare il Prezzo contabile nella
		| Politica contabile o indicare il prezzo sulla scheda del sottoprodotto.';
		|de = 'Der Preis ist für das Nebenprodukt erforderlich. Geben Sie den Abrechnungspreis in
...
Procedure LoadFromFileByProducts(Command)
	...
	DataLoadSettings.Insert("Title", NStr("en = 'Import by-products from file'; ru = 'Загрузить побочную продукцию из файла';pl = 'Importuj produkty uboczne z pliku';es_ES = 'Importar los subproductos del archivo';es_CO = 'Importar los subproductos del archivo';tr = 'Yan ürünleri dosyadan içe aktar';it = 'Importare scarti e residui da file';de = 'Nebenprodukte aus Datei importieren'"));
