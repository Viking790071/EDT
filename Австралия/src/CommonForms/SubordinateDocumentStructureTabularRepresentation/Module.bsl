...
Procedure OutputParentDocuments(DocumentRef)
		...
		For Each CurrentType In TypesAttribute Do
			...
			If AttributeMetadata <> Undefined 
				...
				If AttributeValue <> Undefined 
					AND Not AttributeValue.IsEmpty() 
					AND TypeOf(AttributeValue) = CurrentType 
					AND MAlreadyInList[AttributeValue] = Undefined 
					AND AttributesList.FindByValue(DocumentRef[Attribute.Name]) = Undefined Then
	...
	For Each CWT In DocumentMetadata.TabularSections Do
		...
		If DocumentMetadata = Metadata.Documents.ReconciliationStatement Then			
			Break;			
		EndIf;
...
Procedure OpenDocumentForm()
	...
	Try
		...
		OpeningStructure = New Structure("Key", CurrentData.Ref);
		OpenForm("Document." + CurrentData.DocumentKind + ".ObjectForm", OpeningStructure);
		
	Except
		
		DriveServer.ShowMessageAboutError(DocumentRef, ErrorDescription());
...
Function GetSubordinateDocumentsList(BasisDocument) Export
	...
	For Each ContentItem In Metadata.FilterCriteria.SubordinateDocumentStructure.Content Do
...
Procedure RefreshButtonsEnabled()
	...
	If DataCurrentRows <> Undefined Then
		...
		CommonClientServer.SetFormItemProperty(Items, "Post", "Enabled", PropertyValue);
		CommonClientServer.SetFormItemProperty(Items, "UndoPosting", "Enabled", PropertyValue);
...
Function PostServer(DocumentRef)
	...
	Except
		...
		Message.Text = NStr("en = 'Cannot lock the document.'; ru = 'Невозможно заблокировать документ.';pl = 'Nie można zablokować dokumentu.';es_ES = 'No se puede bloquear el documento.';es_CO = 'No se puede bloquear el documento.';tr = 'Bilgi kilitlenemiyor.';it = 'Non si può bloccare il documento.';de = 'Das Dokument kann nicht gesperrt werden.'");
		Message.Message();
	EndTry; 
...
Function ConvertMultilineRow(MultilineString)
	...
	RowArray = StringFunctionsClientServer.SplitStringIntoSubstringsArray(MultilineString, Chars.LF, True);
	Return StrConcat(RowArray, " ");
...
Procedure Refresh(Command)
	...
	Else
		CloseFormWithWarning(NStr("en = 'The document, for which the report
		                          ...
		                          |ru = 'Документ, для которого сформирован
		                          |отчет о структуре подчиненности был удален, или же стал недоступен.';
		                          |pl = 'Dokument, dla którego wygenerowano
		                          |sprawozdanie dotyczące struktury hierarchii, został usunięty lub jest niedostępny.';
		                          |es_ES = 'El documento, para el cual el informe
		                          |de la estructura de la jerarquía se ha generado, se ha borrado o no se encuentra disponible.';
		                          |es_CO = 'El documento, para el cual el informe
		                          |de la estructura de la jerarquía se ha generado, se ha borrado o no se encuentra disponible.';
		                          |tr = 'Hiyerarşi yapısıyla
		                          | ilgili raporun oluşturulduğu belge silindi veya kullanılamaz.';
		                          |it = 'Il documento, per il quale il report
		                          |sulla struttura gerarchica è stata generato, è stato eliminato o non è disponibile.';
		                          |de = 'Das Dokument, für das der Bericht
		                          |zur Hierarchiestruktur generiert wurde, ist gelöscht oder nicht verfügbar.'"));
	EndIf;		
...
Procedure Post(Button)
	...
	Except
		...
		Message = New UserMessage();
		Message.Text = NStr("en = 'Cannot post the %Document% document.'; ru = 'Не удалось провести документ %Document%!';pl = 'Nie można dekretować dokumentu %Document%.';es_ES = 'No se puede enviar el %Document% documento.';es_CO = 'No se puede enviar el %Document% documento.';tr = '%Document% belgesi kaydedilemiyor.';it = 'Non è possibile pubblicare il documento %Document%.';de = 'Fehler beim Buchen des Dokuments %Document%.'");;
...
Procedure UndoPosting(Button)
	...
	Except
		...
		Message = New UserMessage();
		Message.Text = NStr("en = 'Cannot unpost the %Document% document.'; ru = 'Не удалось сделать непроведенным документ %Document%!';pl = 'Nie udało się zrobić dokument %Document% niezaksięgowanym.';es_ES = 'No se puede reclamar el envío %Document% del documento.';es_CO = 'No se puede reclamar el envío %Document% del documento.';tr = '%Document% belge onaysız hale getirilemedi';it = 'Non è possibile annullare la pubblicazione del documento %Document%.';de = 'Fehler beim Aufheben von Buchung des Dokuments %Document%.'");;
		Message.Text = StrReplace(Message.Text, "%Document%", Items.DocumentsTree.CurrentData.DocumentPresentation);
		Message.Message();
...
Procedure SetDeletionMark(Button)
	...
	Except
		...
		Message = New UserMessage();
		Message.Text = NStr("en = 'Cannot mark the %Document% document for deletion.'; ru = 'Не удалось установить пометку удаления на документ %Document%!';pl = 'Nie można zaznaczyć dokument %Document% do usunięcia.';es_ES = 'No se puede marcar el %Document% documento para borrar.';es_CO = 'No se puede marcar el %Document% documento para borrar.';tr = '%Document% belgesi silinmek üzere işaretlenemiyor.';it = 'Non è possibile contrassegnare il documento %Document% per l''eliminazione.';de = 'Das %Document% Dokument kann nicht zum Löschen markiert werden.'");;
