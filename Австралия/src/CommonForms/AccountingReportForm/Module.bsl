...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ModuleManager = GetReportManagerModule();
	ModuleManager.FillReportFormSettings(ReportSettings);
	
	If ReportSettings.Property("LoadDefaultReportSettings")
		And ReportSettings.LoadDefaultReportSettings Then
		
		ModuleManager.LoadDefaultReportSettings(ThisObject);
...
Procedure OnLoadVariantAtServer(Settings)
	...
	If DetailsMode And Settings <> Undefined Then
...
Procedure OnLoadUserSettingsAtServer(Settings)
	...
	If ReportSettings.Property("UseProcedureOnLoadUserSettingsAtServer")
		And ReportSettings.UseProcedureOnLoadUserSettingsAtServer Then
		
		ModuleManager = GetReportManagerModule();
		ModuleManager.OnLoadUserSettingsAtServer(ThisObject);
...
Procedure Attachable_InputField_OnChange(Item)
	...
	If TypeOf(DCUserSetting) = Type("DataCompositionSettingsParameterValue") Then
		...
		If ReportSettings.Property("ReportFormParameterOnChange")
			And ReportSettings.ReportFormParameterOnChange Then
			
			ParameterOnChangeAtServer(DCUserSetting);
...
&AtServer
Procedure ParameterOnChangeAtServer(DCUserSetting)
	ModuleManager = GetReportManagerModule();
	ModuleManager.ReportFormParameterOnChange(ThisObject, DCUserSetting);
EndProcedure

&AtServer
Function GetReportManagerModule()
	
	Return Common.ObjectManagerByFullName(ReportSettings.FullName);
...
Procedure Attachable_ImportReportOption(Command)
	...
	If FoundItems.Count() = 0 Then
		ShowMessageBox(, NStr("en = 'Report option was not found.'; ru = 'Вариант отчета не найден.';pl = 'Nie znaleziono wariantu raportu.';es_ES = 'Opción del informe no se ha encontrado.';es_CO = 'Opción del informe no se ha encontrado.';tr = 'Rapor seçeneği bulunamadı.';it = 'Variante di Report non è stata trovata.';de = 'Die Berichtsoption wurde nicht gefunden.'"));
		Return;
	EndIf;
...
Procedure BackgroundJobCheckAtClient()
	...
	Else
		If ReportCreated Then
			ShowUserNotification(NStr("en = 'Report is generated'; ru = 'Отчет сформирован';pl = 'Raport został wygenerowany';es_ES = 'Informe generado';es_CO = 'Informe generado';tr = 'Rapor oluşturuldu';it = 'Il report viene generato';de = 'Bericht wurde generiert'"), , Title);
		Else
			ShowUserNotification(NStr("en = 'The report is not generated'; ru = 'Отчет не сформирован';pl = 'Nie wygenerowano raportu';es_ES = 'Informe no generado';es_CO = 'Informe no generado';tr = 'Rapor düzenlenmedi';it = 'Il report non è stato generato';de = 'Der Bericht wurde nicht generiert'"), , Title);
		EndIf;
...
Procedure BeforeGenerateAtClient()
	...
	RunMeasurements = ReportSettings.RunMeasurements AND ValueIsFilled(ReportSettings.MeasurementsKey);
	If RunMeasurements Then
		Comment = ReportSettings.MeasurementsPrefix + "; " + NStr("en = 'Directly:'; ru = 'Непосредственно:';pl = 'Bezpośrednio:';es_ES = 'Directamente:';es_CO = 'Directamente:';tr = 'Doğrudan:';it = 'Direttamente:';de = 'Direkt:'") + " " + String(Directly);
		ModulePerformanceMonitorClient = CommonClient.CommonModule("PerformanceMonitorClient");
		MeasurementID = ModulePerformanceMonitorClient.StartTimeMeasurement(
			False,
...
Procedure SetVisibilityAvailability(UpdateCommands = True)
	...
	EndIf;
	If DetailsMode Then
		Title = Title + " (" + NStr("en = 'Details'; ru = 'Описание';pl = 'Szczegóły';es_ES = 'Detalles';es_CO = 'Detalles';tr = 'Ayrıntılar';it = 'Dettagli';de = 'Details'") + ")";
	EndIf;
EndProcedure
...
Function BackgroundJobStart(Val GeneratingOnOpen, Directly)
	...
	StartParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = 'Report execution: %1'; ru = 'Выполнение отчета: %1';pl = 'Wykonanie raportu: %1';es_ES = 'Realización del informe: %1';es_CO = 'Realización del informe: %1';tr = 'Raporu yerine getirme: %1';it = 'Esecuzione Report: %1';de = 'Ausführung des Berichts: %1'"),
		NameOfReport);
	...
	Else
		...
		StatePresentation.Text                          = NStr("en = 'Generating report...'; ru = 'Отчет формируется...';pl = 'Generowanie raportu...';es_ES = 'Generando el informe...';es_CO = 'Generando el informe...';tr = 'Rapor oluşturma...';it = 'Generazione report...';de = 'Den Bericht wird erstellt...'");
...
Procedure ImportOption(OptionKey)
	...
	StatePresentation.Text     = NStr("en = 'Another report option is selected. Click ""Run report"" to generate the report.'; ru = 'Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';pl = 'Wybrano wariant raportu. Kliknij ""Wygeneruj"", aby otrzymać raport.';es_ES = 'Otra opción del informe se ha seleccionado. Hace clic en ""Generar"" para recibir el informe.';es_CO = 'Otra opción del informe se ha seleccionado. Hace clic en ""Generar"" para recibir el informe.';tr = 'Başka bir rapor seçeneği seçildi. Raporu almak için ""Raporu çalıştır""a tıklayın.';it = 'Un altra variante di report è selezionata. Premere ""Avvia report"" per generare il report.';de = 'Eine andere Berichtsvariante ist ausgewählt. Klicken Sie auf ""Bericht ausführen"", um den Bericht zu generieren.'");
EndProcedure
...
Procedure ShowGenerationErrors(ErrorInformation)
	If TypeOf(ErrorInformation) = Type("ErrorInfo") Then
		...
		DetailedErrorPresentation = NStr("en = 'Generation error:'; ru = 'Ошибка при формировании:';pl = 'Błąd generowania:';es_ES = 'Error de generación:';es_CO = 'Error de generación:';tr = 'Oluşturma hatası:';it = 'Errore di generazione:';de = 'Generierungsfehler:'") + Chars.LF + DetailErrorDescription(ErrorInformation);
