...
Procedure CheckFillOfFormAttributes(Cancel)
	...
	For Each RowPrepayment In Prepayment Do
		...
		If Not ValueIsFilled(RowPrepayment.SettlementsAmount) Then
			Message = New UserMessage();
			Message.Text = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("en = 'The clearing amount is empty in the line %1 of ""To be cleared"" section.'; ru = 'Не заполнена сумма к зачету в строке %1 раздела ""К зачету"".';pl = 'Kwota rozliczenia jest pusta w tym wierszu %1 sekcji ""Do rozliczenia"".';es_ES = 'La cantidad de compensación está vacía en la línea %1 de la sección ""Para borrar"".';es_CO = 'La cantidad de compensación está vacía en la línea %1 de la sección ""Para borrar"".';tr = '“Silinecek” bölümündeki %1 satırındaki mahsup etme tutarı boş. ';it = 'L''importo di compensazione è vuoto nella linea %1 della sezione ""Da compensare"".';de = 'Der Ausgleichsbetrag ist in der Zeile %1 des Abschnitts ""Zu löschen"" leer.'"),
				String(LineNumber));
			Message.Field = "Document";
			Message.Message();
			Cancel = True;
...
Procedure FillAdvances()
	...
	|		WHEN NOT &IsOrder
	|			THEN &Order
	|		WHEN FilteredAdvances.Order = VALUE(Document.SalesOrder.EmptyRef)
	|				OR FilteredAdvances.Order = VALUE(Document.WorkOrder.EmptyRef)
	|				OR FilteredAdvances.Order = VALUE(Document.PurchaseOrder.EmptyRef)
	|			THEN UNDEFINED
	...
	|	-SUM(AccountsReceivableBalances.AmountDocCur) AS AmountDocCur,
	|	-SUM(AccountsReceivableBalances.PaymentAmount) AS PaymentAmount,
	|	CASE
	|		WHEN &ExchangeRateMethod = VALUE(Enum.ExchangeRateMethods.Divisor)
	|			THEN CASE
	|					WHEN SUM(AccountsReceivableBalances.PaymentAmount) <> 0
	|						THEN SUM(AccountsReceivableBalances.SettlementsAmount) / SUM(AccountsReceivableBalances.PaymentAmount)
	|					ELSE 1
	|				END
	|		ELSE CASE
	|				WHEN SUM(AccountsReceivableBalances.SettlementsAmount) <> 0
	|					THEN SUM(AccountsReceivableBalances.PaymentAmount) / SUM(AccountsReceivableBalances.SettlementsAmount)
	|				ELSE 1
	|			END
	|	END AS ExchangeRate,
	|	1 AS Multiplicity
	...
	|		CASE
	|			WHEN &ExchangeRateMethod = VALUE(Enum.ExchangeRateMethods.Divisor)
	|				THEN CAST(AccountsReceivableBalances.AmountCurBalance * &ExchangeRate * &ContractCurrencyMultiplicity / (&ContractCurrencyExchangeRate * &Multiplicity) AS NUMBER(15, 2))
	|			ELSE CAST(AccountsReceivableBalances.AmountCurBalance * &ContractCurrencyExchangeRate * &Multiplicity / (&ExchangeRate * &ContractCurrencyMultiplicity) AS NUMBER(15, 2))
	|		END AS AmountDocCur,
