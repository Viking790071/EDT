...
Procedure AttachFilterLabelsFromArray(Form, FilterFieldName, GroupParentName, LabelsArray) Export
	...
	For Each LabelValue In LabelsArray Do
		CreateLabelItems(Form, FilterFieldName, GroupParentName, LabelValue, String(LabelValue));
	EndDo;
...
Procedure CreateLabelItems(Form, FilterFieldName, GroupParentName, Label, LabelDescription, ListName = "", QueryParameterName="", TPNameLabelData = "LabelData") Export
	...
	If ClientApplication.CurrentInterfaceVariant() <> ClientApplicationInterfaceVariant.Taxi Then
		LabelDescriptionRed = ?(Left(LabelDescription, 16) = LabelDescription, LabelDescription, Left(LabelDescription, 16) + "..");
	Else
		LabelDescriptionRed = ?(Left(LabelDescription, 21) = LabelDescription, LabelDescription, Left(LabelDescription, 21) + "..");;
	EndIf;
	LabelPresentation = FormattedStringLabelPresentation(LabelDescriptionRed, URLFS);
...
Procedure SetListFilter(Form, FilterList, FilterFieldName, ListName = "", UsingFilter=Undefined, TPNameLabelData = "LabelData") Export
	...
	DriveClientServer.SetListFilterItem(FilterList, FilterFieldName, FilterArray, UsingFilter,
...
Procedure CollapseExpandFiltersAtServer(Form, Visible, StructureItemNames = Undefined) Export
	...
	InterfaceTaxi = (ClientApplication.CurrentInterfaceVariant() = ClientApplicationInterfaceVariant.Taxi);
...
Procedure SaveFilterSettings(Val Form, ListName = "", StructureItemNames = Undefined, FormFiltersOption="", SetFilterByPeriod = True, TPNameLabelData = "LabelData") Export
	...
	If StructureItemNames = Undefined Then
		Common.CommonSettingsStorageSave(ObjectKeyName, ObjectKeyName+ListName+"_LabelData", LabelData);
		If SetFilterByPeriod Then
			Common.CommonSettingsStorageSave(ObjectKeyName, ObjectKeyName+ListName+"_FilterByPeriod", Form.FilterPeriod);
		EndIf;
		Common.CommonSettingsStorageSave(ObjectKeyName, ObjectKeyName+ListName+"_FilterPanelVisible", Form.Items.FilterSettingsAndAddInfo.Visible);
	Else
		Common.CommonSettingsStorageSave(ObjectKeyName, ObjectKeyName+ListName+"_LabelData", LabelData);
		If SetFilterByPeriod Then
			Common.CommonSettingsStorageSave(ObjectKeyName, ObjectKeyName+ListName+"_FilterByPeriod", Form[StructureItemNames.FilterPeriod]);
		EndIf; 
		Common.CommonSettingsStorageSave(ObjectKeyName, ObjectKeyName+ListName+"_FilterPanelVisible", Form.Items[StructureItemNames.FilterSettingsAndAddInfo].Visible);
	EndIf;
...
Procedure RestoreFilterSettings(Form, FilterList, ListName = "", 
	...
	SavedValue = Common.CommonSettingsStorageLoad(ObjectKeyName, ObjectKeyName+ListName+"_LabelData");
	...
	If ValueIsFilled(SavedValue) Then
		...
		If SavedValue.Columns.Find("QueryParameterName")=Undefined Then
			...
			For Each FilterField In FilterList.Filter.FilterAvailableFields.Items Do
				ArrayAvailableCompositionSchemaFields.Add(String(FilterField.Field));
			EndDo;
	...
	If SetFilterByPeriod Then
		If StructureItemNames = Undefined Then
			Form.FilterPeriod = Common.CommonSettingsStorageLoad(ObjectKeyName, ObjectKeyName+ListName+"_FilterByPeriod");
		...
		Else
			Form[StructureItemNames.FilterPeriod] = Common.CommonSettingsStorageLoad(ObjectKeyName, ObjectKeyName+ListName+"_FilterByPeriod");
	...
	If Not IsCurrentListFilter AND (Not SetFilterByPeriod Or Not ValueIsFilled(Form.FilterPeriod)) Then
		SavedValue = Common.CommonSettingsStorageLoad(ObjectKeyName, ObjectKeyName+ListName+"_VisibilityFilterPanel", True);
		If ValueIsFilled(SavedValue) Then
