...
Procedure Paste(Object, TPName, FormItems, CopiedCount, PastededCount) Export
	...
	For Each TPAttribute In TPMetadata.Attributes Do
		...
		FixedChoiceParameterPresentation = "Row." + TPAttribute.Name + ".";
		...
		For Each ChoiceParameter In ChoiceParameters Do
			...
			ChoiceParameterPresentation = FixedChoiceParameterPresentation + AttributePresentation;
			...
			If TypeOf(ChoiceParameter.Value) = Type("FixedArray") OR TypeOf(ChoiceParameter.Value) = Type("Array") Then
				For Each AttributeValue In ChoiceParameter.Value Do
					...
					AcceptableValues.Add(AttributeValue);
				...
				EndDo;
			Else
				AcceptableValues.Add(ChoiceParameter.Value);
				AttributeCondition = AttributeCondition + ChoiceParameterPresentation + "=AcceptableValues[" + ValuesIterator + "] ";
				ValuesIterator = ValuesIterator + 1;
			EndIf;
			...
			If AttributeTypes.Count() > 1 Then
				...
				For Each Type In AttributeTypes Do
					...
					If Common.IsCatalog(MetadataObjectByType)
						OR Common.IsDocument(MetadataObjectByType) Then
						...
						If MetadataObjectByType.Attributes.Find(AttributePresentation) = Undefined Then
							...
							AcceptableValues.Add(Type);
							AttributeConditionByTypeOR = AttributeConditionByTypeOR + "TypeOf(String." + TPAttribute.Name + ")" + "=AcceptableValues[" + ValuesIterator + "] ";
							ValuesIterator = ValuesIterator + 1;
				...
				AttributeConditionByTypeAND = AttributeConditionByTypeAND + ")";
				AttributeCondition = "(" + AttributeConditionByTypeAND + " AND (" + AttributeCondition + ")) OR " + AttributeConditionByTypeOR;
				
			EndIf;
			
			AddedRows = FindByCondition(AddedRows, TPAttribute.Name, AttributeCondition, AcceptableValues);
