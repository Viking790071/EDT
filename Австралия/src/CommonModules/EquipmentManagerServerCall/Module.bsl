...
Function GetBarcodePicture(BarcodeParameters) Export
	...
	If ExternalComponent = Undefined Then
		Raise NStr("en = 'An error occurred while connecting external barcode printing component.'; ru = 'Ошибка подключения внешней компоненты печати штрихкода!';pl = 'Podczas podłączenia zewnętrznego komponentu druku kodów kreskowych wystąpił błąd.';es_ES = 'Ha ocurrido un error al conectar el componente externo de la impresión del código de barras.';es_CO = 'Ha ocurrido un error al conectar el componente externo de la impresión del código de barras.';tr = 'Harici barkod yazdırma bileşeni bağlanırken bir hata oluştu.';it = 'Si è verificato un errore durante la connessione al dispositivo esterno per la stampa di codici a barre.';de = 'Beim Anschließen der externen Barcode-Druckkomponente ist ein Fehler aufgetreten.'");
	EndIf;
	...
	ExternalComponent.CodeAuto = False;
	...
	If BarcodeParameters.CodeType = 99 Then
		...
		Else
			ExternalComponent.CodeAuto = True;
		EndIf;
	Else
		ExternalComponent.CodeAuto = False;
	...
	ExternalComponent.TextVisible = BarcodeParameters.ShowText;
	
	// Form a barcode image
	ExternalComponent.CodeValue = BarcodeParameters.Barcode;
	
	If BarcodeParameters.Property("CanvasRotation") Then
		ExternalComponent.CanvasRotation = BarcodeParameters.CanvasRotation;
	Else
		ExternalComponent.CanvasRotation = 0;
	EndIf;
	
	// If the specified width is less than minimal for this barcode.
	If ExternalComponent.Width < ExternalComponent.CodeMinWidth Then
		ExternalComponent.Width = ExternalComponent.CodeMinWidth;
	EndIf;
	
	// If the defined height is less than minimal for this barcode.
	If ExternalComponent.Height < ExternalComponent.CodeMinHeight Then
		ExternalComponent.Height = ExternalComponent.CodeMinHeight;
	EndIf;

	If BarcodeParameters.Property("SizeOfFont") AND (BarcodeParameters.SizeOfFont > 0) 
		AND (BarcodeParameters.ShowText) AND (ExternalComponent.FontSize <> BarcodeParameters.SizeOfFont) Then
		ExternalComponent.FontSize = BarcodeParameters.SizeOfFont;
	EndIf;
