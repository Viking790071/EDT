...
Function SegmentsCounterparties(Segments) Export
	...
	For Each Segment In Segments Do
		...
		If Counterparties.Count() = 0 Then
			Counterparties = SegmentContent;
		Else
			Counterparties = DriveClientServer.GetMatchingArraysItems(Counterparties, SegmentContent);
		EndIf;
...
Function CounterpartiesByTagsAndSegments(Tags, Segments) Export
	...
	If Tags.Count() > 0 AND Segments.Count() > 0 Then
		Counterparties = DriveClientServer.GetMatchingArraysItems(CounterpartiesByTags(Tags), SegmentsCounterparties(Segments));
...
Procedure GenerateSegments(SegmentRef = Undefined) Export

	ContactsClassification.GenerateCounterpartySegments(SegmentRef);
