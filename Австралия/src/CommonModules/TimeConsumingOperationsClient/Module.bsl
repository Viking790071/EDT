...
Procedure WaitForCompletion(Val TimeConsumingOperation, Val CompletionNotification = Undefined, 
	...
	If FormParameters.OutputIdleWindow Then
		...
		OpenForm("CommonForm.TimeConsumingOperation", FormParameters, 
	...
	Else
		...
		AttachIdleHandler("TimeConsumingOperationMonitor", FormParameters.CurrentInterval, True);
	EndIf;
...
//
// Parameters:
//  OwnerForm - ClientApplicationForm, Undefined - the form used to call the time-consuming operation.
//
// Returns:
...
////////////////////////////////////////////////////////////////////////////////
// Procedures and functions for the time-consuming operation form.
//
...
// Opens the time-consuming operation progress form.
// 
// Parameters:
//  FormOwner        - ClientApplicationForm - the form used to open the time-consuming operation progress form.
...
// Closes the time-consuming operation progress form.
// 
// Parameters:
//  TimeConsumingOperationForm - ClientApplicationForm - the reference to the time-consuming operation indication form. 
//
...
Procedure CheckParametersWaitForCompletion(Val TimeConsumingOperation, Val CompletionNotification, Val IdleParameters)
	...
	CommonClientServer.CheckParameter("TimeConsumingOperationsClient.WaitForCompletion",
	...
	If IdleParameters <> Undefined Then
		...
		CommonClientServer.Validate(IdleParameters.Interval = 0 Or IdleParameters.Interval >= 1, 
			NStr("ru = 'Параметр IdleParameters.Interval должен быть больше или равен 1'; en = 'The IdleParameters.Interval parameter must be equal to or greater than 1.'; pl = 'Opcja IdleParameters.Interval musi być większy lub równy 1.';es_ES = 'El parámetro IdleParameters.Interval debe ser mayor o igual al 1.';es_CO = 'El parámetro IdleParameters.Interval debe ser mayor o igual al 1.';tr = 'BeklemeParametreleri.Aralık parametresi birden fazla veya 1 olmalıdır.';it = 'Parametro ParametriAttesa.Intervallo deve essere maggiore o uguale a 1.';de = 'Parameter WarteParameter.Das Intervall muss größer oder gleich 1 sein.'"),
			"TimeConsumingOperationsClient.WaitForCompletion");
		CommonClientServer.Validate(Not (IdleParameters.ExecutionProgressNotification <> Undefined AND IdleParameters.OutputIdleWindow), 
			NStr("ru = 'Если параметр IdleParameters.OutputIdleWindow установлен в True, то параметр IdleParameters.ExecutionProgressNotification не поддерживается'; en = 'If the IdleParameters.OutputIdleWindow parameter is True, the IdleParameters.ExecutionProgressNotification parameter is ignored.'; pl = 'Jeśli parametr IdleParameters.OutputIdleWindma wartość True, IdleParameters.ExecutionProgressNotification parameter jest ignorowany.';es_ES = 'Si el parámetro IdleParameters.OutputIdleWindow es True, se ignora el parámetro IdleParameters.ExecutionProgressNotification.';es_CO = 'Si el parámetro IdleParameters.OutputIdleWindow es True, se ignora el parámetro IdleParameters.ExecutionProgressNotification.';tr = 'BeklemeParametreleri.BeklemePenceresiniGöster parametresi Doğru olarak ayarlanırsa, BeklemeParametreleri.GidişatBildirimi parametresi desteklenmiyor';it = 'Se il parametro ParametriDiAttesa.MostrareFinestraDiAttesa è impostato a True, allora il parametro PatametriDiAttesa.AvvisoSulProgressoDiCompletamento non è supportato.';de = 'Wenn die Option IdleParameters.OutputIdleWindow auf True gesetzt ist, wird die Option IdleParameters.ExecutionProgressNotification nicht unterstützt'"),
			"TimeConsumingOperationsClient.WaitForCompletion");
	EndIf;
