...
Procedure GetExchangePlans(SubsystemExchangePlans) Export
	...
	DataExchangeDrive.GetExchangePlans(SubsystemExchangePlans);
...
// If errors occur on sending the data, handler execution must be stopped using the Raise method 
// with an error description.
//
// Parameters:
//
//  StandardProcessing - Boolean - this parameter stores the flag of whether the standard (system) 
//                                 event processing is executed.
//   If this parameter is set to False in the processing procedure, standard processing is skipped.
//    Canceling standard processing does not mean canceling the operation.
//   Default value is True.
//
//  Recipient - ExchangePlanRef - an exchange plan node for which data is exported.
//
//  MessageFileName  - String - a name of the file to export data to.
//   If this parameter is filled, the platform expects data to be exported to file.
...
//
//  MessageData - Arbitrary - if the MessageFileName parameter is empty, the system expects data to 
//   be exported to this parameter.
//
//  TransactionItemsCount - Number - defines the maximum number of data items that can be placed in 
//   the message within a single database transaction.
//   You can implement the algorithms of transaction locks for the data being exported in this 
//   handler.
//   The value of this parameter is set in the data exchange subsystem settings.
//
//  LogEventName - String - a name of an event log entry for the current data exchange session.
...
//   It is used to count the number of sent objects. The number is then written to the exchange 
//   protocol.
//
...
// If errors occur on receiving data, the handler execution must be stopped using the Raise method 
// with an error description.
//
// Parameters:
//
...
//   If the MessageFileName parameter is empty, the data is to be imported using this parameter.
//   
//
//  TransactionItemsCount - Number - defines the maximum number of data items that can be read from 
//   a message and recorded to the infobase within a single transaction.
...
//   It matches the EventName parameter of the WriteLogEvent method of the global context.
//
//  ReceivedObjectCount - Number - a counter of received objects.
...
// This handler can improve initial data export performance using exchange plan filters for 
// restricting data migration.
// Registering changes with data migration restriction filters must be implemented in this handler.
// You can use the DataExchangeServer.RegisterDataByExportStartDateAndCompany universal procedure 
// 
// if the data migration is restricted by date or by date and company.
// The handler cannot be used for performing exchange in a distributed infobase.
// 
...
//   Default value is True.
//
//  Filter - Array or MetadataObject - determines filter for the metadata objects to register 
//                                       changes for.
//
...
// The event occurs if an object is modified both in the current infobase and in the correspondent 
// infobase, and the modifications are different.
// It overrides the standard data change conflict processing.
// The standard processing of conflicts implies receiving changes from the master node and ignoring 
// changes from a subordinate node.
...
// objects, or infobase object properties, or source nodes, or the entire infobase, or all data 
// items.
// This handler is called on data exchange execution of any type (on data exchange in a distributed 
// infobase and on data exchange based on exchange rules).
//
// Parameters:
//  DataItem - Arbitrary - a data item that is read from the data exchange message.
//                  Data items can be ConstantValueManager.<Constant name>, infobase objects (except 
//                  for Object deletion), register record sets, sequences, or recalculations.
//                  
//
//  GetItem - GetDataItem - defines whether a read data item is to be recorded to the infobase if a 
//                                               conflict occurs.
//   On calling the handler, the default parameter value is Auto, and this implies receiving data 
//   from the master node and ignoring data from the subordinate node.
...
//
//  DataReceivingFromMasterNode - Boolean -  in a distributed infobase, this flag shows whether data 
...
//   In exchanges based on exchange rules, its value is True if the object priority specified in the 
//   exchange rules and used for resolving conflicts is set to Higher (the default value) or not specified.
//   This parameter value is False if the object priority is set to Lower or Equal.
//   In other cases, the parameter value is True.
//
Procedure OnDataChangeConflict(Val DataItem, GetItem, Val Sender, Val DataReceivingFromMasterNode) Export
...
//
// Parameters:
//   FormatVersions - Map - map of format version number to the common module where the export or 
//                   import handlers for this version are located.
//
// Example:
