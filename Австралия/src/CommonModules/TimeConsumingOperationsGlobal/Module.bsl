...
Procedure TimeConsumingOperationMonitor() Export
	
	TimeConsumingOperationsInProgress = TimeConsumingOperationsClient.TimeConsumingOperationsInProgress();
...
Procedure MonitorTimeConsumingOperations(TimeConsumingOperationsInProgress)
	...
	For each TimeConsumingOperation In TimeConsumingOperationsInProgress Do
		...
		EndIf;
		If TimeConsumingOperation.CompletionNotification <> Undefined AND TypeOf(TimeConsumingOperation.CompletionNotification.Module) = Type("ClientApplicationForm") 
	...
	Statuses = New Map;
	Statuses = TimeConsumingOperationsServerCall.ActionsCompleted(JobsToCheck, JobsToCancel);
	...
	AttachIdleHandler("TimeConsumingOperationMonitor", Interval, True);
...
Function MonitorTimeConsumingOperation(TimeConsumingOperation, Status)
	...
	If Status.Status = "Completed" Then
		...
		TimeConsumingOperationsClient.ShowNotification(TimeConsumingOperation.UserNotification);
