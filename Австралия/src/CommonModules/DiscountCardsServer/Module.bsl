...
Function GetDiscountCardBasicCodeType() Export
	...
	|	DiscountCardTypes.CardType AS CardType
	|FROM
...
Function FindDiscountCards(CardCode, CodeType, Pattern = Undefined) Export
	...
	Query = New Query(
	"SELECT ALLOWED DISTINCT
	|	DiscountCardTypes.Ref AS Ref,
	|	DiscountCardTypes.ThisIsMembershipCard AS ThisIsMembershipCard,
	|	DiscountCardTypes.CardType AS CardType,
	|	DiscountCardTypes.DiscountCardTemplate,
	|	DiscountCardTypes.DeletionMark
	|INTO CardsKinds
	|FROM
	|	Catalog.DiscountCardTypes AS DiscountCardTypes
	|WHERE
	|	(DiscountCardTypes.CardType = &MixedCardType
	|			OR DiscountCardTypes.CardType = &CardType)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT ALLOWED
	...
	While Selection.Next() Do
		...
		Else
			...
			// A user will enter the code manually, so it can not be checked for compliance with the template We do not know what
			// the magnetic track contains.Namely we will give an opportunity to choose any kind of a discount card.
