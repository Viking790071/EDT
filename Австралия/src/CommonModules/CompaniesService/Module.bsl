...
//                 shall list the types that correspond to the field used in the template of access restriction and
//                 paired with access type "Object". When listing the types by access type "Object", you should list only
//                 those types of flieds that the field InformationRegisters.AccessValueSets.Object has, the rest of
//                 types is irrelevant.
// 
...
//  - ValueGroupType        - Type - Reference type of access values groups. For example, Type("CatalogRef.ProductsAccessGroups").
//  - SeveralGroupsOfValues - Boolean - True shows that for access value (Products) 
//                            several value groups can be selected (Products access group).
//
...
#Region ConditionalCallsOfTheSSLSubsystems
