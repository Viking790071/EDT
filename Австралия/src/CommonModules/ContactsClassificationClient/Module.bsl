...
Function CreatedFilterClick(Form, ListName, PeriodVariant, Item) Export
	...
	For IndexOf = 0 To Form.FilterCreated.Count()-1 Do
		...
		If PeriodsListItem.Check Then
			PeriodDisplayItem.BackColor = CommonClientCached.StyleColor("FilterActiveValueBackground");
		Else
	...
	GenerateFilterOptionTitle(Form.Items.FilterPeriod, CommonClientServer.MarkedItems(Form.FilterCreated).Count());
	
	FilterGroupPeriod = CommonClientServer.CreateFilterItemGroup(Form[ListName].Filter.Items, "FilterPeriod", DataCompositionFilterItemsGroupType.AndGroup);
...
Function TagFilterClick(Form, ListName, Item, StandardProcessing) Export
	...
	Tags = CommonClientServer.MarkedItems(Form.FilterTags);
	Segments = CommonClientServer.MarkedItems(Form.FilterSegments);
	...
	CommonClientServer.SetDynamicListFilterItem(Form[ListName], "Ref", Counterparties, DataCompositionComparisonType.InList, , FilterIsOn);
...
Function SegmentFilterClick(Form, ListName, Item, StandardProcessing) Export
	...
	If Item.Name = "Segment_Explanation" Then
		OpenParameters = New Structure("Title, ToolTipKey", 
			"How to work with counterparty segments",
...
Procedure SelectFilterVariant(Form, Command) Export
	...
	Else
		...
		Items[Command.Name].BackColor = CommonClientCached.StyleColor("FilterActiveValueBackground");
	EndIf;
