...
Procedure PeriodPresentationSelectPeriod(Form, FilterListName="List", FieldFilterName="Date", StructureItemNames = Undefined, NotificationPeriodChanged = Undefined) Export
	...
	If Not (NotificationPeriodChanged = Undefined) Then
		Parameters.Insert("NotificationPeriodChanged", NotificationPeriodChanged);
	EndIf;
...
Procedure PeriodPresentationClickCompleted(NewPeriod, Parameters) Export
	...
	Else
		...
		If Parameters.Property("NotificationPeriodChanged") Then
			Notify(Parameters.NotificationPeriodChanged);
		EndIf;
