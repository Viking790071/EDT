...
Procedure OnCreateAtServer(FilledObject, DataLoadSettings, ThisObject, UseFormSSL = True) Export
	...
	DataLoadSettings.Insert("UseTogether", UseTogetherWithShippedSSLPart() AND UseFormSSL AND Not RunMode.IsWebClient);
	
	If RunMode.IsWebClient 
	...
	Else
		...
		DataImportMethodFromExternalSources= DriveReUse.GetValueOfSetting("DataImportMethodFromExternalSources");
		If Not ValueIsFilled(DataImportMethodFromExternalSources) Then
			
			DataImportMethodFromExternalSources= Enums.DataImportMethodFromExternalSources.FileChoice;
			DriveServer.SetUserSetting(DataImportMethodFromExternalSources, "DataImportMethodFromExternalSources");
	...
	IsChartOfAccountsImport = (Find(FillingObjectFullName, "ChartOfAccounts") > 0);
	DataLoadSettings.Insert("IsChartOfAccountsImport", IsChartOfAccountsImport);
	
	IsAccountingEntriesImport = (Find(FillingObjectFullName, "AccountingRegister") > 0);
	DataLoadSettings.Insert("IsAccountingEntriesImport", IsAccountingEntriesImport);
	
EndProcedure

Procedure AddImportDescriptionField(ImportFieldsTable, FieldName, FieldPresentation, FieldType, DerivedValueType,
	FieldsGroupName = "", FieldsGroupPresentation = "", Priority = 0, RequiredFilling = False, GroupRequiredFilling = False, Visible = True,
	AdditionalAttributeFeature = False, AdditionalAttributeRef = Undefined) Export
	...
	NewRow.FieldsGroupPresentation 		= FieldsGroupPresentation;
...
Procedure CreateFieldsAndGroupsTree(GroupsAndFields)
	...
	GroupsAndFields.Columns.Add("AdditionalAttributeRef",		New TypeDescription("ChartOfCharacteristicTypesRef.AdditionalAttributesAndInfo"));
...
Procedure CreateImportDescriptionFieldsTable(ImportFieldsTable) Export
	...
	ImportFieldsTable.Columns.Add("FieldsGroupPresentation");
	...
	ImportFieldsTable.Columns.Add("AdditionalAttributeRef", New TypeDescription("ChartOfCharacteristicTypesRef.AdditionalAttributesAndInfo"));
...
Procedure GetManagerByFillingObjectName(FillingObjectFullName, Manager) Export
	...
	Manager = Common.ObjectManagerByFullName(FillingObjectFullName);
...
Procedure FillInGroupAndFieldsTree(ImportFieldsTable, GroupsAndFields, FillingObjectFullName, ThisIsImportToTP)
	...
	FieldGroupsTable = ImportFieldsTable.Copy( , "FieldsGroupName, FieldsGroupPresentation");
	FieldGroupsTable.GroupBy("FieldsGroupName, FieldsGroupPresentation");
	...
	For Each TableRow In FieldGroupsTable Do
		...
		If IsBlankString(TableRow.FieldsGroupPresentation) Then
			NewFirstLevelRow.FieldPresentation = TableRow.FieldsGroupName;
		Else
			NewFirstLevelRow.FieldPresentation = TableRow.FieldsGroupPresentation;
		EndIf;
...
Procedure CreateAndFillGroupAndFieldsByObjectNameTree(DataLoadSettings, GroupsAndFields, ThisIsImportToTP = False, ImportFieldsTable = Undefined) Export
...
Procedure ProcessSelectedAdditionalAttributes(CatalogItem, IsExistingObject, TableRow, SelectedAdditionalAttributes) Export
	...
	For Each AdditionalAttributeDescription In SelectedAdditionalAttributes Do
		...
		ElsIf NOT IsBlankString(StringValue) Then
			...
			If ValueTypeArray.Find(Type("CatalogRef.ObjectsPropertiesValues")) <> Undefined Then
				
				DataImportFromExternalSourcesOverridable.CreateAdditionalProperty(AdditionalAttributeValue, Property, False, StringValue);
				
			ElsIf ValueTypeArray.Find(Type("CatalogRef.ObjectPropertyValueHierarchy")) <> Undefined Then
...
// Work and services; Designing of air-conditioning and ventilation systems;
// where second and fourth parameter will be skipped instead of filling in an empty value.
//
Function SplitStringIntoSubstringsArray(Val String, Delimiter) Export
...
Procedure ImportCSVFileToTabularDocument(TempFileName, SpreadsheetDocument)
	...
	ListSeparator = DriveServer.GetListSeparator(String);
	
	While String <> Undefined Do
		
		FirstColumn = True;
		
		SubstringArray = SplitStringIntoSubstringsArray(String, ListSeparator);
...
Procedure FillInDetailsInTabularDocument(SpreadsheetDocument, ColumnsCount, DataLoadSettings) Export
	...
	Details.Add("Do not import", NStr("en = 'Do not import'; ru = 'Не загружать';pl = 'Nie importować';es_ES = 'No importar';es_CO = 'No importar';tr = 'İçe aktarma';it = 'Non importare';de = 'Nicht importieren'"));
	...
	For ColumnNumber = 1 To ColumnsCount Do
		...
		SpreadsheetDocument.InsertArea(HeaderArea.Areas.Title, DestinationArea, SpreadsheetDocumentShiftType.Vertical, True);
		
		DestinationArea.Text 				= NStr("en = 'Do not import'; ru = 'Не загружать';pl = 'Nie importować';es_ES = 'No importar';es_CO = 'No importar';tr = 'İçe aktarma';it = 'Non importare';de = 'Nicht importieren'");
...
Function GetChartOfAccountsData(ChartOfAccounts) Export
	
	Return Common.ObjectAttributesValues(ChartOfAccounts, "UseQuantity, UseAnalyticalDimensions, TypeOfEntries");
