...
Function GetParameterValue(Object, Document = Undefined, Parameter, PresentationParameter = Undefined) Export
	...
	ElsIf Parameter = Parameters.CompanyHead Then
		...
		ResponsiblePersons = DriveServer.OrganizationalUnitsResponsiblePersons(Company, CurrentSessionDate());
	...
	ElsIf Parameter = Parameters.Facsimile Then 
		...
		Return StringFunctionsClientServer.SubstituteParametersToString("<span style='background-color: #DCDCDC'>%1</span>",
			NStr("en = 'Facsimile is not set for company'; ru = 'Не указано факсимилье для компании';pl = 'Nie ustawiono faksymile dla firmy';es_ES = 'Facsímil no está establecido para la empresa';es_CO = 'Facsímil no está establecido para la empresa';tr = 'İş yeri için faks ayarlanmamış';it = 'Il facsimile non è impostato per l''azienda';de = 'Faksimile ist nicht für Firma festgelegt'"));
	...
	ElsIf Parameter = Parameters.Logo Then 
		...
		Return StringFunctionsClientServer.SubstituteParametersToString("<span style='background-color: #DCDCDC'>%1</span>",
			NStr("en = 'Logo is not set for company'; ru = 'Не указан логотип компании';pl = 'Nie ustawiono logo dla firmy';es_ES = 'Logotipo no está establecido para la empresa';es_CO = 'Logotipo no está establecido para la empresa';tr = 'İş yeri için logo ayarlanmadı';it = 'Il logo non è impostato per l''azienda';de = 'Logo ist nicht für die Firma festgelegt'"));
...
Function GetAdditionalAttributeValue(Object, Document, AttributeRef) Export
	...
	If AttributeRef.PropertySet = Catalogs.AdditionalAttributesAndInfoSets.Catalog_CounterpartyContracts Then
		ValuesOfAdditionalAttributes = Object.AdditionalAttributes;
	ElsIf AttributeRef.PropertySet = Catalogs.AdditionalAttributesAndInfoSets.Catalog_Counterparties Then
		ValuesOfAdditionalAttributes = Object.Owner.AdditionalAttributes;
	ElsIf AttributeRef.PropertySet = Catalogs.AdditionalAttributesAndInfoSets.Document_SalesOrder Then
		If Document <> Undefined Then
			ValuesOfAdditionalAttributes = Document.AdditionalAttributes;
		Else
			ValuesOfAdditionalAttributes = Undefined;
		EndIf;
	ElsIf AttributeRef.PropertySet = Catalogs.AdditionalAttributesAndInfoSets.Document_Quote Then
		If Document <> Undefined Then
			ValuesOfAdditionalAttributes = Document.AdditionalAttributes;
		Else
			ValuesOfAdditionalAttributes = Undefined;
	...
	ElsIf AttributeType = Type("CatalogRef.ObjectsPropertiesValues")
		Or AttributeType = Type("CatalogRef.ObjectPropertyValueHierarchy") Then 
		If AttributeDataTable.Count() > 0 Then 
			Return AttributeDataTable[0].Value.Description;
		Else 
			Return "";
...
Function EmptyDocumentField()
	...
	|<body>
	|</br>
	|<p align='center'><font color='grey'>%1</font></p>
	|</body>
	|</html>";
	
	Return StringFunctionsClientServer.SubstituteParametersToString(
		EmptyDocumentField,
		NStr("en = 'Specify form treaty'; ru = 'Укажите договор формы';pl = 'Określ formularz orozumienia';es_ES = 'Especifique el formato del tratado';es_CO = 'Especifique el formato del tratado';tr = 'Anlaşmadan belirt';it = 'Specificare modulo contratto';de = 'Geben Sie das Vertragsformular an'"));
...
Procedure DeclineValue(Parameter, ParameterValue, Case)
	If Parameter = Enums.ContractsWithCounterpartiesTemplatesParameters.PositionOfContactPersonOfCounterparty Then
		...
		ArrayOfWords = StringFunctionsClientServer.SplitStringIntoSubstringsArray(ParameterValue, " ", True, True);
		...
		ParameterValue = StrConcat(DeclinedWords, " ");
	Else
