...
Procedure CalculateOrdersFulfilmentSchedule()
	...
	|	PurchaseOrdersBalances.Products AS Products,
...
Procedure GenerateSourceRecordsTable(Cancel, Replacing)
	...
	If Not StructureTemporaryTables.Property("RegisterRecordsPurchaseOrdersChange") OR
		...
		|	PurchaseOrders.Products AS Products,
	...
	Else
		...
		// If the "RegisterRecordsSalesOrdersChange" temporary table exists and contains records on changes of the set,
		// it means the set is written not for the first time and the set was not controlled for balances.
		// Current state of the set and current state of changes are placed into the "RegisterRecordsSalesOrdersBeforeWrite"
		// temporary table to get the change of a new set with respect to the initial set when writing.
...
Procedure GenerateRecordsChangeTable(Cancel, Replacing)
	...
	|		RegisterRecordsPurchaseOrdersBeforeWrite.Products AS Products,
	...
	|		RegisterRecordsPurchaseOrdersOnWrite.Products,
	...
	|	RegisterRecordsPurchaseOrdersChange.Products,
