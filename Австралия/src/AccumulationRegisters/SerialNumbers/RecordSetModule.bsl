...
Procedure BeforeWrite(Cancel, Replacing)
	...
	If NOT StructureTemporaryTables.Property("RegisterRecordsSerialNumbersChange") OR
		...
		// If the "RegisterRecordsSerialNumbersInWarrantyChange" temporary table does not exists
		// or does not contain records on the set change, it means that the set is written for the first time or the set was
		// controlled for balances. Current state of the set is placed
		// into the "RegisterRecordsSerialNumbersInWarrantyBeforeWrite" temporary table in order to get the change of a new
		// set with respect to the current one when writing.
...
Procedure OnWrite(Cancel, Replacing)
	...
	Query = New Query(
	"SELECT
	|	MIN(RegisterRecordsSerialNumbersChange.LineNumber) AS LineNumber,
	|	RegisterRecordsSerialNumbersChange.Products AS Products,
	|	RegisterRecordsSerialNumbersChange.Characteristic AS Characteristic,
	|	RegisterRecordsSerialNumbersChange.Batch AS Batch,
	|	RegisterRecordsSerialNumbersChange.Ownership AS Ownership,
	...
	|	RegisterRecordsSerialNumbersChange.StructuralUnit AS StructuralUnit,
	|	RegisterRecordsSerialNumbersChange.Cell AS Cell,
	|	SUM(RegisterRecordsSerialNumbersChange.QuantityBeforeWrite) AS QuantityBeforeWrite,
	|	SUM(RegisterRecordsSerialNumbersChange.QuantityChange) AS QuantityChange,
	|	SUM(RegisterRecordsSerialNumbersChange.QuantityOnWrite) AS QuantityOnWrite
	|INTO RegisterRecordsSerialNumbersChange
	|FROM
	|	(SELECT
	|		RegisterRecordsSerialNumbersBeforeWrite.LineNumber AS LineNumber,
	|		RegisterRecordsSerialNumbersBeforeWrite.Products AS Products,
	|		RegisterRecordsSerialNumbersBeforeWrite.Characteristic AS Characteristic,
	|		RegisterRecordsSerialNumbersBeforeWrite.Batch AS Batch,
	|		RegisterRecordsSerialNumbersBeforeWrite.Ownership AS Ownership,
	|		RegisterRecordsSerialNumbersBeforeWrite.SerialNumber AS SerialNumber,
	|		RegisterRecordsSerialNumbersBeforeWrite.StructuralUnit AS StructuralUnit,
	|		RegisterRecordsSerialNumbersBeforeWrite.Cell AS Cell,
	|		RegisterRecordsSerialNumbersBeforeWrite.QuantityBeforeWrite AS QuantityBeforeWrite,
	|		RegisterRecordsSerialNumbersBeforeWrite.QuantityBeforeWrite AS QuantityChange,
	|		0 AS QuantityOnWrite
	|	FROM
	|		RegisterRecordsSerialNumbersBeforeWrite AS RegisterRecordsSerialNumbersBeforeWrite
	|	
	|	UNION ALL
	|	
	|	SELECT
	|		RegisterRecordsSerialNumbersOnWrite.LineNumber,
	|		RegisterRecordsSerialNumbersOnWrite.Products,
	|		RegisterRecordsSerialNumbersOnWrite.Characteristic,
	|		RegisterRecordsSerialNumbersOnWrite.Batch,
	|		RegisterRecordsSerialNumbersOnWrite.Ownership,
	...
	|		0,
	|		CASE
	|			WHEN RegisterRecordsSerialNumbersOnWrite.RecordType = VALUE(AccumulationRecordType.Receipt)
	|				THEN -RegisterRecordsSerialNumbersOnWrite.Quantity
	|			ELSE RegisterRecordsSerialNumbersOnWrite.Quantity
	|		END,
	|		RegisterRecordsSerialNumbersOnWrite.Quantity
	|	FROM
	|		AccumulationRegister.SerialNumbers AS RegisterRecordsSerialNumbersOnWrite
	|	WHERE
	|		RegisterRecordsSerialNumbersOnWrite.Recorder = &Recorder) AS RegisterRecordsSerialNumbersChange
	|
	|GROUP BY
	|	RegisterRecordsSerialNumbersChange.Products,
	|	RegisterRecordsSerialNumbersChange.Characteristic,
	|	RegisterRecordsSerialNumbersChange.Batch,
	|	RegisterRecordsSerialNumbersChange.Ownership,
	...
	|
	|HAVING
	|	SUM(RegisterRecordsSerialNumbersChange.QuantityChange) <> 0
	|
	|INDEX BY
	|	Products,
	|	Characteristic,
