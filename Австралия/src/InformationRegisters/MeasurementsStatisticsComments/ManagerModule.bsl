	...
	If Not Measurements.IsEmpty() Then
		...
		While Selection.Next() Do
			If Selection.StatisticsComment <> ZeroID Then
				...
				NewRecord1.PeriodEnd1 = Selection.PeriodEnd1;
			EndIf;
		EndDo;
...
Function GetAggregatedRecords(AggregationBoundary, ProcessRecordsUntil, AggregationPeriod, DeletionPeriod) Export
	...
	|	DATEADD(DATETIME(2015,1,1),SECOND, CAST((DATEDIFF(DATETIME(2015,1,1), MeasurementsStatisticsComments.Period, SECOND) + 63555667200)/&AggregationPeriod - 0.5 AS NUMBER(11,0)) * &AggregationPeriod - 63555667200 + &AggregationPeriod - 1) AS PeriodEnd1,
...
Function GetMeasurements(StartDate, EndDate, DeletionPeriod) Export
	...
	|	AND MeasurementsStatisticsComments.PeriodEnd1 <= &EndDate
	|	AND MeasurementsStatisticsComments.DeletionID < DATEADD(DATETIME(2015,1,1),SECOND, CAST((DATEDIFF(DATETIME(2015,1,1), &EndDate, SECOND) + 63555667200)/&DeletionPeriod - 0.5 AS NUMBER(11,0)) * &DeletionPeriod - 63555667200)
	|GROUP BY
	|	DATEADD(DATETIME(2015,1,1),SECOND, CAST((DATEDIFF(DATETIME(2015,1,1), MeasurementsStatisticsComments.Period, SECOND) + 63555667200)/&DeletionPeriod - 0.5 AS NUMBER(11,0)) * &DeletionPeriod - 63555667200),
