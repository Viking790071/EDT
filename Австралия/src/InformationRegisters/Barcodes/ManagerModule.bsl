...
Function GetDataByBarCodes(Barcodes) Export
	...
	While Selection.Next() Do
		...
		CurData.Insert("Products", Selection.Products);
...
Function GetMaximumValueOfBarcodeCodeNumber(PrefixOfPieceProduct = "0", PrefixOfInnerBarcode = "00") Export
	...
	Query = New Query(
		...
		|WHERE
		|	Barcodes.Barcode LIKE &BarcodeTemplate"
	);
...
//  String
//
Function GenerateBarcodeTransportWeightGoodsEAN13(WeightProductPrefix = "1", MaximumCode = 99999) Export
...
Function GetWeightBarcodeMaximumCodeValueAsNumber(WeightProductPrefix = "1") Export
	...
	Query = New Query("
	|SELECT
	|	MAX(SubString(Barcodes.Barcode, 3, 5)) AS Code
	|FROM
	|	InformationRegister.Barcodes AS Barcodes
	|WHERE
	|	Barcodes.Barcode LIKE &BarcodeFormat
	|");

	Query.SetParameter("BarcodeFormat", InformationRegisters.Barcodes.WeightBarcodeFormat(WeightProductPrefix));
...
Function GetPrefixesWeightBarcodes() Export
	...
	Query = New Query(
	"SELECT DISTINCT
	|	ExchangeWithOfflinePeripheralsRules.WeightProductPrefix AS Prefix
	|FROM
...
Procedure ConvertWeightBarcode(CurBarcode) Export
	...
	If StrLen(CurBarcode.Barcode) = 13 // EAN13
		...
		InternalBarcode13 = InternalBarcode12 + InformationRegisters.Barcodes.CheckCharacterEAN(InternalBarcode12, 13);
