...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Filter.Property("GLAccount") Then
		Raise NStr("en = 'The report is not intended for direct usage'; ru = 'Отчет не предназначен для непосредственного использования';pl = 'Raport nie jest przeznaczony dla bezpośredniego użycia';es_ES = 'El informe no está destinado para el uso directo';es_CO = 'El informe no está destinado para el uso directo';tr = 'Rapor doğrudan kullanım için uygun değildir';it = 'Il report non è inteso per un utilizzo diretto';de = 'Der Bericht ist nicht für den direkten Gebrauch bestimmt'");
	EndIf;
	...
	For Each DCUserSetting In DCUserSettings.Items Do
		...
		If Type = "SettingsParameterValue"
			...
			If PeriodBeginning.AutoMarkIncomplete
				...
				ErrorText = NStr("ru = 'Не указан период.'; en = 'The period is not specified.'; pl = 'Okres nie jest określony.';es_ES = 'Período no está especificado.';es_CO = 'Período no está especificado.';tr = 'Dönem belirtilmedi.';it = 'Il periodo non è specificato.';de = 'Der Zeitraum ist nicht angegeben.'");
...
Procedure Attachable_ImportReportOption(Command)
	...
	If FoundItems.Count() = 0 Then
		ShowMessageBox(, NStr("ru = 'Вариант отчета не найден.'; en = 'Report option was not found.'; pl = 'Nie znaleziono wariantu raportu.';es_ES = 'Opción del informe no se ha encontrado.';es_CO = 'Opción del informe no se ha encontrado.';tr = 'Rapor seçeneği bulunamadı.';it = 'Variante di Report non è stata trovata.';de = 'Die Berichtsvariante wurde nicht gefunden.'"));
		Return;
	EndIf;
...
Procedure BackgroundJobCheckAtClient()
	...
	Else
		If ReportCreated Then
			ShowUserNotification(NStr("ru = 'Отчет сформирован'; en = 'Report is generated'; pl = 'Raport został wygenerowany';es_ES = 'Informe generado';es_CO = 'Informe generado';tr = 'Rapor oluşturuldu';it = 'Il report viene generato';de = 'Bericht wird generiert'"), , Title);
		Else
			ShowUserNotification(NStr("ru = 'Отчет не сформирован'; en = 'The report is not generated'; pl = 'Nie wygenerowano raportu';es_ES = 'Informe no generado';es_CO = 'Informe no generado';tr = 'Rapor oluşturulmadı';it = 'Il report non è stato generato';de = 'Der Bericht ist nicht generiert'"), , Title);
		EndIf;
...
Procedure SetVisibilityAvailability(UpdateCommands = True)
	...
	EndIf;
	If DetailsMode Then
		Title = Title + " (" + NStr("ru = 'Подробнее'; en = 'Details'; pl = 'Szczegóły';es_ES = 'Detalles';es_CO = 'Detalles';tr = 'Ayrıntılar';it = 'Dettagli';de = 'Details'") + ")";
	EndIf;
EndProcedure
...
Function BackgroundJobStart(Val GeneratingOnOpen, Directly)
	...
	Else
		...
		StatePresentation.Text                          = NStr("ru = 'Отчет формируется...'; en = 'Generating report...'; pl = 'Generowanie raportu...';es_ES = 'Generando el informe...';es_CO = 'Generando el informe...';tr = 'Rapor oluşturuluyor...';it = 'Generazione report...';de = 'Den Bericht wird generiert...'");
