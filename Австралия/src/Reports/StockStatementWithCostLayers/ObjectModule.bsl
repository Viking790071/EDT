...
Function PrepareReportParameters(ReportSettings)
	...
	Period = Date(1,1,1);
	TitleOutput = False;
	Title = NStr("en = 'Stock statement (with cost layers)'; ru = 'Остатки запасов (по партиям)';pl = 'Stany magazynowe (z warstwami kosztów własnych)';es_ES = 'Declaración del stock (con capas del coste)';es_CO = 'Declaración del stock (con capas del coste)';tr = 'Stok durumu (maliyet katmanları ile)';it = 'Resoconto dello stock (con livelli di costo)';de = 'Bestandsabrechnung (mit Kostenebenen)'");
	...
	If ParameterPeriod <> Undefined AND ParameterPeriod.ViewMode = DataCompositionSettingsItemViewMode.QuickAccess Then
		...
		If ParameterPeriod.Use Then
			If TypeOf(ParameterPeriod.Value) = Type("StandardBeginningDate") Then
				...
				Period = ParameterPeriod.Value.Date;
			Else
	...
	ReportParameters.Insert("ReportId", "StockStatementWithCostLayers");
