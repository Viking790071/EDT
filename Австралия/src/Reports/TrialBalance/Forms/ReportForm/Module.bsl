...
Function RefreshReport() Export 
	...
	If BeginOfPeriod <> Undefined AND EndOfPeriod <> Undefined AND BeginOfPeriod > EndOfPeriod Then		
		...
		Message.Text = NStr("en = 'The Accounting period start date must precede or match its end date.'; ru = 'Дата начала отчетного периода должна быть менее даты его окончания.';pl = 'Data początku okresu rozliczeniowego powinna poprzedzać datę zakończanie lub odpowiadać jej.';es_ES = 'La fecha de inicio del Período contable debe preceder o coincidir con su fecha final.';es_CO = 'La fecha de inicio del Período contable debe preceder o coincidir con su fecha final.';tr = 'Hesap döneminin başlangıç tarihi bitiş tarihinden sonra olamaz.';it = 'La data di inizio del Periodo contabile deve essere precedente o corrispondente alla sua data di fine.';de = 'Das Startdatum des Abrechnungszeitraums soll vor oder an dessen Enddatum liegen.'");
		Message.Message();		
		Return Undefined;	
	EndIf; 
	...
	// Set order by a period of records (in seconds) for a recorder drill down
	DataCompositionGroup = DataCompositionSettings.Structure[0];
	If DataCompositionGroup.GroupFields.Items.Count() 
		And DataCompositionGroup.GroupFields.Items[0].Field = New DataCompositionField("Recorder") Then
		
		DataCompositionGroup.Order.Items.Clear();
		
		OrderItem = DataCompositionGroup.Order.Items.Add(Type("DataCompositionOrderItem"));
		OrderItem.Field = New DataCompositionField("SecondPeriod");
		
	EndIf;
	
	Schema = Reports.TrialBalance.GetTemplate("MainDataCompositionSchema");
	...
	DetailsData = PutToTempStorage(CompositionDetailsData, UUID);
	CompositionSchema = PutToTempStorage(Schema, UUID);
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	IsAccountingCurrency = Constants.ForeignExchangeAccounting.Get();
    ...
    If ActionsPerformedByParameter <> Undefined Then
        ...
        If ExecutedAction = DataCompositionDetailsProcessingAction.OpenValue Then
            ...
            If ValueIsFilled(ActionsPerformedByParameter) Then
                ...
                If TypeOf(ActionsPerformedByParameter) = Type("ChartOfAccountsRef.PrimaryChartOfAccounts") Then
                    
                    OpenForm("ChartOfAccounts.PrimaryChartOfAccounts.Form.AccountForm", DetailsParameter);
        ...
        ElsIf ExecutedAction = DataCompositionDetailsProcessingAction.DrillDown
            ...
            CompositionSettings = PutToTempStorage(NewCompositionSettings, UUID);
            
            ParametersStructure = New Structure;
            ParametersStructure.Insert("EncryptingMode", True);
            ParametersStructure.Insert("CompositionSettings", CompositionSettings);
            
            OpenForm("Report.TrialBalance.Form.ReportForm", ParametersStructure, , New UUID);
