...
Procedure OnComposeResult(ResultDocument, DetailsData, StandardProcessing)
	...
	ParameterCurrency = ReportSettings.DataParameters.FindParameterValue(New DataCompositionParameter("Currency"));
	If ParameterCurrency <> Undefined AND NOT ValueIsFilled(ParameterCurrency.Value) Then
		ParameterCurrency.Value = DriveReUse.GetFunctionalCurrency();
	EndIf;
	...
	While True Do
		...
		Else
			// Fix header
			If NOT TableFixed AND ResultItem.ParameterValues.Count() > 0 
...
Function PrepareReportParameters(ReportSettings)
	...
	TitleOutput = False;
	Title = NStr("en = 'Invoices valid for EPD'; ru = 'Инвойсы, доступные для скидки за досрочную оплату';pl = 'Zestawienie skonta';es_ES = 'Facturas válidas para EPD';es_CO = 'Facturas válidas para EPD';tr = 'Erken ödeme indirimi için geçerli faturalar';it = 'Fatture valide per EPD';de = 'Rechnungen gültig für Skonto'");
	...
	ReportParameters.Insert("ReportId", "InvoicesValidForEPD");
