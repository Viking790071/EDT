...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.Property("PrintParameters") Then
		PrintParameters = Parameters.PrintParameters;
		ID = PrintParameters.ID;
		PrintManager = PrintParameters.PrintManager;
		
		If PrintParameters.Property("AdditionalParameters") Then
			AdditionalParameters = PrintParameters.AdditionalParameters;
			
			If AdditionalParameters.Property("Result")
				And AdditionalParameters.Result <> Undefined Then
				LanguageCode = AdditionalParameters.Result.LanguageCode;
			EndIf;
		EndIf;
		
	EndIf;
	
    Items.FormSettings.Visible = Parameters.PrintParameters.Property("ID")
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	ElsIf Upper(ChoiceSource.FormName) = Upper("CommonForm.SelectAttachmentFormat") Then
		...
		If ValueSelected <> Undefined AND ValueSelected <> DialogReturnCode.Cancel Then
			...
			EmailOperationsClient.CreateNewEmailMessage(NewLettersParameters);
		EndIf;
...
Procedure Save(Command)
	...
	OpenForm("CommonForm.SavePrintForm", , ThisObject);
...
Procedure Send(Command)
	...
	OpenForm("CommonForm.SelectAttachmentFormat", , ThisObject);
...
Procedure Settings(Command)
	...
	AdditionalParameters = New Structure("MetadataObject, Result", CounterpartiesToPrint[0].Value, Undefined);
	PrintParameters = New Structure("ID, PrintManager, AdditionalParameters", ID, PrintManager, AdditionalParameters);
	
	OpeningParameters = New Structure("PrintManagerName,TemplatesNames,CounterpartiesInitialValue,PrintParameters, UniqueKey");
    OpeningParameters.TemplatesNames				= ID;
    OpeningParameters.PrintManagerName				= PrintManager;
    OpeningParameters.CounterpartiesInitialValue	= CounterpartiesToPrint.UnloadValues();
    OpeningParameters.PrintParameters				= PrintParameters;
    OpeningParameters.UniqueKey						= UniqueKey;
                    
    NotifyDescription = New NotifyDescription("AfterSettings", ThisObject, OpeningParameters);
    Form = OpenForm("DataProcessor.PrintOptions.Form.Form", PrintParameters,,,,,NotifyDescription);
...
Procedure RefreshContactInformationOnServer()
	...
	If PrintOption = "Card" Then
		Spreadsheet = GeneratePrintedFormCard(CounterpartiesToPrint.UnloadValues(), LanguageCode);
	ElsIf PrintOption = "List" Then
		Spreadsheet = GeneratePrintableFormList(CounterpartiesToPrint.UnloadValues(), LanguageCode);
	EndIf;
...
//  SpreadsheetDocument 
&AtServerNoContext
Function GeneratePrintedFormCard(Counterparties, LanguageCode)
	...
	If Not ValueIsFilled(LanguageCode) Then
		LanguageCode = DriveServer.GetCurrentUserLanguageCode();
	EndIf;
	...
	AreaStructure = FillStructureLayoutRegions("Card", LanguageCode);
	...
	For Each Counterparty In Counterparties Do
		...
		If SelectionCounterparties.FindNext(SearchForCounterparty) Then
			...
			AreaStructure.CounterpartyData.Parameters.TINPresentation = ?(SettingsCIContent.CounterpartyTIN,
				NStr("en = 'TIN'; ru = 'ИНН';pl = 'NIP';es_ES = 'NIF';es_CO = 'NIF';tr = 'VKN';it = 'Cod.Fiscale';de = 'Steuernummer'", LanguageCode) + " " + SelectionCounterparties.TINPresentation, "");
...
//  SpreadsheetDocument 
&AtServerNoContext
Function GeneratePrintableFormList(Counterparties, LanguageCode)
	...
	AreaStructure = FillStructureLayoutRegions("List", LanguageCode);
	SettingsCIContent = LoadSettingsCIContent();
	...
	For Each Counterparty In Counterparties Do
		...
		If SelectionCounterparties.FindNext(SearchForCounterparty) Then
			...
			If CounterpartyRecordNo % 2 = 0 Then
				...
				Area.BackColor = StyleColors.ContactInformationFormColor;
			EndIf;
...
Function GetCounterpartiesToPrint(CounterpartiesList)
	...
	Counterparties = CommonClientServer.MarkedItems(CounterpartiesList);
...
// 		contact persons, ResponsibleManager - output flag of Responsible person.
&AtServerNoContext
Function LoadSettingsCIContent()
	...
	While Selection.Next() Do
		...
		If UseCI = Undefined Then
			UseCI = DriveServer.SetPrintDefaultCIKind(Selection.CIKind);
		EndIf;
	...
	SettingsCIContent.CounterpartyTIN = Common.CommonSettingsStorageLoad("ManagementStructureTheContactInformationOfTheCounterparty",
		"CounterpartyTIN", True);
		
	SettingsCIContent.MainContactPerson = Common.CommonSettingsStorageLoad("ManagementStructureTheContactInformationOfTheCounterparty",
		"MainContactPerson", True);
		
	SettingsCIContent.OtherContactPersons = Common.CommonSettingsStorageLoad("ManagementStructureTheContactInformationOfTheCounterparty",
		"OtherContactPersons", True);
		
	SettingsCIContent.ResponsibleManager = Common.CommonSettingsStorageLoad("ManagementStructureTheContactInformationOfTheCounterparty",
...
//
Function FillStructureLayoutRegions(PrintOption, LanguageCode)
	
	AreaStructure = New Structure;
	
	If PrintOption = "Card" Then
		
		Template = PrintManagement.PrintFormTemplate("Catalog.Counterparties.PF_MXL_ContactInformationCard", LanguageCode);
	...
	ElsIf PrintOption = "List" Then
		...
		Template = PrintManagement.PrintFormTemplate("Catalog.Counterparties.PF_MXL_ContactInformationList", LanguageCode);
...
Function GetTempFileNameForPrintForm(TemplateName, Extension, UsedFilesNames)
	...
	TempFileName = CommonClientServer.ReplaceProhibitedCharsInFileName(
		StringFunctionsClientServer.SubstituteParametersToString(FileNamePattern, TemplateName, "", Extension));
...
Procedure SavePrintFormsToFolder(FilesListInTempStorage, Val Folder = "")
	...
	Status(NStr("en = 'Saving completed successfully.'; ru = 'Сохранение успешно завершено.';pl = 'Zapisywanie zakończyło się pomyślnie.';es_ES = 'Se ha guardado con éxito.';es_CO = 'Se ha guardado con éxito.';tr = 'Kaydetme tamamlandı.';it = 'Salvataggio completato con successo.';de = 'Speichern erfolgreich abgeschlossen.'"), , NStr("en = 'to folder:'; ru = 'в папку:';pl = 'do folderu:';es_ES = 'a la carpeta:';es_CO = 'a la carpeta:';tr = 'klasöre:';it = 'nella cartella:';de = 'zum Ordner:'") + " " + Folder);
