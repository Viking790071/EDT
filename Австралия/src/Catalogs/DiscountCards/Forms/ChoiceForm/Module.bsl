...
Procedure FilterCardKindOnChange(Item)
	...
	DriveClientServer.SetListFilterItem(List, "Owner", FilterCardKind, ValueIsFilled(FilterCardKind));
...
Procedure FilterOnlyNominalOnChange(Item)
	...
	If FilterOnlyNominal Then
		...
		DriveClientServer.SetListFilterItem(List, "CardOwner", Parameters.Counterparty, True);
	Else
		List.Filter.Items.Clear();
		VL = New ValueList;
		...
		DriveClientServer.SetListFilterItem(List, "CardOwner", VL, True, DataCompositionComparisonType.InList);
	EndIf;
...
Procedure FilterBarcodeOnChange(Item)
	...
	DriveClientServer.SetListFilterItem(List, "CardCodeBarcode", FilterBarcode, ValueIsFilled(FilterBarcode), DataCompositionComparisonType.Contains);
...
Procedure FilterMagneticCodeOnChange(Item)
	...
	DriveClientServer.SetListFilterItem(List, "CardCodeMagnetic", FilterMagneticCode, ValueIsFilled(FilterMagneticCode), DataCompositionComparisonType.Contains);
	...
	If ReceivedCodeType = PredefinedValue("Enum.CardCodesTypes.MagneticCode") Then
		...
		Else
			...
			ElsIf Result.Count() = 0 Then
				DriveClient.ShowMessageAboutError(ThisObject, "Card code does not correspond to any template of magnetic cards.");
			Else
