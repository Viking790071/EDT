	...
	If Not FormIsClosing AND Not Exit Then
		If WereMadeChanges Then
			...
			ShowQueryBox(New NotifyDescription("BeforeClosingQueryBoxHandler", ThisObject),
					NStr("en = 'Add selected variants to document?'; ru = 'Добавить выбранные варианты в документ?';pl = 'Dodać wybrane warianty do dokumentu?';es_ES = '¿Añadir las variantes seleccionadas al documento?';es_CO = '¿Añadir las variantes seleccionadas al documento?';tr = 'Seçilen varyantlar belgeye eklensin mi?';it = 'Aggiungere le varianti selezionate al documento?';de = 'Ausgewählte Varianten zum Dokument hinzufügen?'"),
...
&AtClient
Procedure ListOnActivateCell(Item)
	
	Items.ShoppingCart.CurrentRow = Items.List.CurrentRow;
...
Procedure ListSelection(Item, SelectedRow, Field, StandardProcessing)
	...
	NameParts = StrSplit(Field.Name, "_");
	
	If NameParts.Count() = 2 Then
		
		ColumnName = NameParts[1];
		
		If ColumnName <> "RowName" And List[SelectedRow][ColumnName] <> "-" Then
			
			StandardProcessing = False;
			
			StartValue = ?(ShoppingCart[SelectedRow][ColumnName] > 0, ShoppingCart[SelectedRow][ColumnName], 1);
			
			AdditionalParameters = New Structure;
			AdditionalParameters.Insert("RowNumber", SelectedRow);
			AdditionalParameters.Insert("ColumnName", ColumnName);
			AdditionalParameters.Insert("StartValue",
				?(ShoppingCart[SelectedRow][ColumnName] > 0, ShoppingCart[SelectedRow][ColumnName], 0));
			
			Description = New NotifyDescription("AfterInputQuantity", ThisObject, AdditionalParameters);
			ShowInputNumber(Description, StartValue, NStr("en = 'Input count'; ru = 'Входное количество';pl = 'Ilość wejść';es_ES = 'Conteo de entradas';es_CO = 'Conteo de entradas';tr = 'Giriş sayısı';it = 'Conteggio input';de = 'Eingangsabrechnung'"), 10);
...
&AtClient
Procedure ShoppingCartOnActivateCell(Item)
	
	If Items.ShoppingCart.CurrentItem <> Undefined Then
		...
		If NameParts.Count() = 2 Then
			...
			If ColumnName <> "RowName" Then
				...
				ShoppingCartStartValue = ShoppingCart[SelectedRow][ColumnName];
				ShoppingCartRowNumber = SelectedRow;
				ShoppingCartColumnName = ColumnName;
				
				Items.List.CurrentRow = SelectedRow;
...
Procedure ShoppingCartOnChange(Item)
	...
	StrListField = List[ShoppingCartRowNumber][ShoppingCartColumnName];
	StrArray = StrSplit(StrListField, "/");
	
	If StrArray.Count() = 2 Then
		
		OldAvailable = StringFunctionsClientServer.StringToNumber(StrArray[0]);
			
		NewAvailable = OldAvailable
			+ ShoppingCartStartValue
			- ShoppingCart[ShoppingCartRowNumber][ShoppingCartColumnName];
		
		List[ShoppingCartRowNumber][ShoppingCartColumnName] = "" + NewAvailable + "/" + StrArray[1];
		...
		If ColumnValues.Count() Then
			...
			Filter.Name = List[ShoppingCartRowNumber].RowName;
...
&AtClient
Procedure AfterInputQuantity(Result, AdditionalParameters) Export
	
	If Result <> Undefined Then
		
		RowNumber = AdditionalParameters.RowNumber;
		ColumnName = AdditionalParameters.ColumnName;
		
		ShoppingCart[RowNumber][ColumnName] = Result;
		
		StrListField = List[RowNumber][ColumnName];
		StrArray = StrSplit(StrListField, "/");
		...
		If StrArray.Count() = 2 Then
			...
			NewAvailable = OldAvailable + AdditionalParameters.StartValue - Result;
			
			List[RowNumber][ColumnName] = "" + NewAvailable + "/" + StrArray[1];
			
			// Selected variants
			Filter = New Structure;
			Filter.Insert("Name", ColumnName);
			ColumnValues = ColumnsMap.FindRows(Filter);
			If ColumnValues.Count() Then
				ColumnValue = ColumnValues[0].Value;
				
				Filter.Name = List[RowNumber].RowName;
				RowValues = RowsMap.FindRows(Filter);
	...
	If AdditionalAttributes.Count() = 2 Then
		...
		If ColumnsCharacteristic.AdditionalValuesUsed Then
			...
			While SelectionDetailRecords.Next() Do
				...
				Matrix.Columns.Add(ColumnMap.Name, TypeDescriptionS10, SelectionDetailRecords.Description);
		...
		Else
			...
			While SelectionDetailRecords.Next() Do
				...
				Matrix.Columns.Add(ColumnMap.Name, TypeDescriptionS10, String(ColumnMap.Value));
...
Procedure SetConditionalAppearance()
	...
	ListColumns = List.Unload().Columns;
	For Each Column In ListColumns Do
		...
		If Column.Name = "RowName" Then
			...
			ItemField.Field = New DataCompositionField("List_" + Column.Name);
		...
		Else
			...
			FilterItem.LeftValue		= New DataCompositionField("List." + Column.Name);
	...
	While SelectionDetailRecords.Next() Do
		...
		If LinesWithRowName.Count() > 0 Then
			...
			If RowsToFill.Count() > 0 Then
				...
				RowID = RowToFill.GetID();
				CartRowToFill = ShoppingCart.FindByID(RowID);
				...
				If LinesWithColumnName.Count() > 0 Then
					...
					Available = SelectionDetailRecords.Available - CartRowToFill[ColumnName];
					RowToFill[ColumnName] = "" + Available + "/" + SelectionDetailRecords.InStock;
