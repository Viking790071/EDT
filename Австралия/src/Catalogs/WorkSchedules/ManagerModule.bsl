...
Procedure WriteScheduleDataToRegister(WorkSchedule, DateOfFilling) Export
	...
	PeriodsTable = WorkSchedule.DailySchedules.Unload();
	PeriodsTable.Sort("DayNumber Desc");
	MaxNumberOfDay = 0;
	...
	CalendarData = DriveServer.ReadScheduleDataFromRegister(BaseCalendar, Year(DateOfFilling));
	...
	For Ct = FirstDay To LastDay Do
		...
		For Each CurRow In WorkSchedule.DailySchedules Do
...
Procedure ChoiceDataGetProcessing(ChoiceData, Parameters, StandardProcessing)
	...
	NativeLanguagesSupportServer.ChoiceDataGetProcessing(
		...
		Parameters,
		StandardProcessing,
		Metadata.Catalogs.WorkSchedules);
