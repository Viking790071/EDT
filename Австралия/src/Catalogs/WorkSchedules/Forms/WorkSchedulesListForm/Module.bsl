...
Procedure SetConditionalAppearance()
	...
	ColorPalePink	= StyleColors.NonWorkingTimeDayOff;
	
	For NumberCounter = 1 To 31 Do
		
		StrNumber = Format(NumberCounter, "ND=2");
		SetAppearanceField("DayOfWeek", StrNumber, ColorPalePink);
		
	EndDo;
	
	For NumberCounter = 28 To 31 Do
		
		StrNumber = Format(NumberCounter, "ND=2");
		SetAppearanceField("IsDate", StrNumber, ColorPalePink);
...
&AtServer
Function SetAppearanceField(StringAppearance, StrNumber, ColorBackground)
	
	If StringAppearance = "DayOfWeek" Then
		...
		DataFilterItem.LeftValue		= New DataCompositionField("ResourcesBySchedule.DayOfWeek"+StrNumber);
		DataFilterItem.ComparisonType	= DataCompositionComparisonType.Equal;
		DataFilterItem.RightValue		= 6;
		DataFilterItem.Use				= True;
		
		DataFilterItem					= GroupFilterItem.Items.Add(Type("DataCompositionFilterItem"));
		DataFilterItem.LeftValue		= New DataCompositionField("ResourcesBySchedule.DayOfWeek"+StrNumber);
		DataFilterItem.ComparisonType	= DataCompositionComparisonType.Equal;
		DataFilterItem.RightValue		= 7;
		DataFilterItem.Use				= True;
		
		ItemAppearance.Appearance.SetParameterValue("BackColor", ColorBackground);
		
		FieldAppearance = ItemAppearance.Fields.Items.Add();
		FieldAppearance.Field = New DataCompositionField("ResourcesByScheduleNumber"+StrNumber);
		FieldAppearance.Use = True;
		
	ElsIf StringAppearance = "IsDate" Then
		
		ItemAppearance = ConditionalAppearance.Items.Add();
		
		DataFilterItem					= ItemAppearance.Filter.Items.Add((Type("DataCompositionFilterItem")));
		DataFilterItem.LeftValue		= New DataCompositionField("ResourcesBySchedule.IsDate"+StrNumber);
		DataFilterItem.ComparisonType	= DataCompositionComparisonType.Equal;
...
Procedure DisplayTimetableScheduleYear()
	...
	CalendarData = DriveServer.ReadScheduleDataFromRegister(BaseCalendar, Year(ScheduleDate));
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ScheduleDate = CurrentSessionDate();
...
Procedure TimetableScheduleCreate(Command)
	...
	OpenForm("InformationRegister.WorkSchedulesOfResources.RecordForm", New Structure("WorkSchedule", Items.List.CurrentRow));
