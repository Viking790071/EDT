...
Procedure ChoiceDataGetProcessing(ChoiceData, Parameters, StandardProcessing)
	...
	NativeLanguagesSupportServer.ChoiceDataGetProcessing(
		...
		Parameters,
		StandardProcessing,
		Metadata.Catalogs.POSTerminals);
...
//
// Parameters:
//  Company - CatalogRef.CashAccounts (CatalogRef.CashRegisters) - Ref to petty cash
//
// Returns:
...
Function GetFeeData(POSTerminal, ChargeCardKind) Export
	
	Result = New Structure("FeePercent, FeeFixedPart", 0, 0);
	
	Query = New Query;
	Query.Text =
	"SELECT ALLOWED TOP 1
	|	POSTerminalsPaymentCardKinds.FeePercent AS FeePercent,
	|	POSTerminalsPaymentCardKinds.FeeFixedPart AS FeeFixedPart
	|FROM
	|	Catalog.POSTerminals.PaymentCardKinds AS POSTerminalsPaymentCardKinds
	|WHERE
	|	POSTerminalsPaymentCardKinds.Ref = &Ref
	|	AND POSTerminalsPaymentCardKinds.ChargeCardKind = &ChargeCardKind";
	
	Query.SetParameter("Ref", POSTerminal);
	Query.SetParameter("ChargeCardKind", ChargeCardKind);
