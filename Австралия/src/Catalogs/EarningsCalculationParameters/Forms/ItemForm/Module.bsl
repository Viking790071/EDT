...
// current pages for the visible management depending on the attribute value of the SpecifyValueAtPayrollCalculation
// catalog item
//
...
Procedure GenerateQueryFillParameters()
	...
	QueryText = "SELECT 
					|	"+ FieldList + "
					|FROM 
					|	"+ Object.SourceName + " AS Source" + ?(ConditionsList = "", "", "
					|WHERE 
					|	"+ ConditionsList);
	...
	For Each FilterItem In DataFilter.Settings.Filter.Items Do
		If Object.QueryParameters.FindRows(New Structure("Name", String(FilterItem.LeftValue))).Count() = 0 Then
			...
			NewRow.ComparisonType = Common.ValueToXMLString(FilterItem.ComparisonType);
...
Function QueryCorrect()
	...
	Try
		...
		If QueryBuilder.Dimensions.Count() > 0 Then
			...
			Message.Text = NStr("en = 'Query should not contain totals.'; ru = 'Запрос не должен содержать итоги!';pl = 'Zapytanie nie powinno zawierać wyników.';es_ES = 'Solicitud no tiene que contener totales.';es_CO = 'Solicitud no tiene que contener totales.';tr = 'Sorgu toplamları içermemelidir.';it = 'La Query non deve contenere i totali.';de = 'Die Abfrage darf keine Summen enthalten.'");
			Message.Message();
			Return False;
		EndIf; 
		If QueryBuilder.SelectedFields.Count() > 1 Then
			Message = New UserMessage();
			Message.Text = NStr("en = 'The request cannot contain more than one indicator.'; ru = 'Запрос должен содержать не более одного показателя!';pl = 'Zapytanie nie może zawierać więcej niż jeden wskaźnik.';es_ES = 'La solicitud no puede contener más de un indicador.';es_CO = 'La solicitud no puede contener más de un indicador.';tr = 'İstek birden fazla göstergeyi içeremez.';it = 'La richiesta non può contenere più di un indicatore.';de = 'Die Anfrage darf nicht mehr als einen Indikator enthalten.'");
			Message.Message();
			Return False;
		EndIf;
		Return True;
...
Function CheckForIDDuplication(Cancel)
	...
	Query = New Query(
	"SELECT
	|	EarningsCalculationParameters.ID
	|FROM
	|	Catalog.EarningsCalculationParameters AS EarningsCalculationParameters
	|WHERE
	|	EarningsCalculationParameters.ID = &ID
	|	AND EarningsCalculationParameters.Ref <> &Ref");
	...
	If Cancel Then
	  	...
	  	Message = New UserMessage();
		Message.Text = NStr("en = 'The calculation parameter with the same Identifier already exists.'; ru = 'Параметр расчета с таким идентификатором уже существует.';pl = 'Parametr obliczenia z takim samym identyfikatorem już istnieje.';es_ES = 'El parámetro de cálculo con el mismo Identificador ya existe.';es_CO = 'El parámetro de cálculo con el mismo Identificador ya existe.';tr = 'Aynı kimliğe sahip hesaplama parametresi zaten mevcut.';it = 'Il parametro di calcolo con lo stesso identificativo esiste già.';de = 'Der Berechnungsparameter mit der gleichen ID existiert bereits.'");
		Message.Message();
...
Function CheckForIndicatorChoice()
	...
	Message = New UserMessage();
	Message.Text = NStr("en = 'Indicator is not selected.'; ru = 'Не выбран показатель!';pl = 'Nie wybrano wskaźnika.';es_ES = 'Indicador no se ha seleccionado.';es_CO = 'Indicador no se ha seleccionado.';tr = 'Gösterge seçilmedi.';it = 'Indicatore non selezionato.';de = 'Indikator ist nicht ausgewählt.'");
	Message.Message();
...
Procedure FillIndicatorsAndSourceDataFilterPeriods()
	...
	If Periodical Then
		...
		If Find(Object.SourcePresentation, "turnovers") > 0 OR Find(Object.SourcePresentation, "movement:") > 0 Then
			...
			FilterNewBoundary.PeriodBoundaryType		= Enums.PeriodBoundaryTypes.BeginOfPeriod;

			FilterNewBoundary = Object.DataFilterPeriods.Add();
			FilterNewBoundary.BoundaryDateName			= "EndOfPeriod";
			FilterNewBoundary.BoundaryDatePresentation = "Data filter end date";
			FilterNewBoundary.PeriodBoundaryType		= Enums.PeriodBoundaryTypes.EndOfPeriod;
	...
	If Not Object.CustomQuery Then
		...
		For Each ParameterString In Object.QueryParameters Do
			...
			If ParameterString.Name <> "PointInTime"
				...
				Else
					FilterItem.ComparisonType = Common.ValueFromXMLString(ParameterString.ComparisonType);
				EndIf; 
...
Procedure SourceStartChoice(Item, ChoiceData, StandardProcessing)
	...
	OpenForm("Catalog.EarningsCalculationParameters.Form.SourceChoiceForm",,,,,,Notification);
	...
	If Object.CustomQuery Then
		...
		If EditQuery Then
			If Not QueryCorrect() Then
				...
				ShowQueryBox(New NotifyDescription("EditQueryEnd", ThisObject), NStr("en = 'Query contains error. Clear query and return to original one?'; ru = 'Запрос содержит ошибку! Очистить запрос и вернуться к исходному?';pl = 'Zapytanie zawiera błąd. Oczyścić zapytanie i wrócić do oryginału?';es_ES = 'Solicitud contiene un error. ¿Eliminar la solicitud y volver a la original?';es_CO = 'Solicitud contiene un error. ¿Eliminar la solicitud y volver a la original?';tr = 'Sorgu hata içeriyor. Sorgu temizlensin ve orijinal haline dönsün mü?';it = 'La query contiene un errore. Cancellare la query e tornare all''originale?';de = 'Die Abfrage enthält einen Fehler. Abfrage löschen und zur ursprünglichen zurückkehren?'"), QuestionDialogMode.YesNo, 0);
				Return;
			Else
...
Procedure SpecifyValueAtPayrollCalculationOnChange(Item)
	...
	If Object.SpecifyValueAtPayrollCalculation Then
		...
		MessageText = NStr("en = 'After the check box is selected, all attributes will be cleared. Continue?'; ru = 'После установки флага все реквизиты будут очищены! Продолжить?';pl = 'Po zaznaczeniu tego pola wyboru wszystkie atrybuty zostaną oczyszczone. Kontynuować?';es_ES = 'Después de haber seleccionado la casilla de verificación, todos atributos se eliminarán. ¿Continuar?';es_CO = 'Después de haber seleccionado la casilla de verificación, todos atributos se eliminarán. ¿Continuar?';tr = 'Onay kutusu seçildikten sonra, tüm öznitelikler silinecek. Devam edilsin mi?';it = 'Dopo aver impostato il contrassegno, tutti i requisiti saranno cancellati! Continuare?';de = 'Nachdem das Kontrollkästchen aktiviert wurde, werden alle Attribute gelöscht. Fortfahren?'");
...
#Region FillingByTemplates

&AtClient
Procedure CreateFixedAmount(Command)
	
	Notification = New NotifyDescription("FillByTemplateEnd",ThisForm,"FixedAmount");
	ShowQueryBox(Notification,NStr("en = 'Calculation parameter will be completely repopulated. Continue?'; ru = 'Параметр расчета будет полностью перезаполнен! Продолжить?';pl = 'Parametr obliczeniowy zostanie całkowicie ponownie wypełniony. Kontynuować?';es_ES = 'Parámetro de cálculo se volverá a poblar completamente. ¿Continuar?';es_CO = 'Parámetro de cálculo se volverá a poblar completamente. ¿Continuar?';tr = 'Hesaplama parametresi tamamen yeniden doldurulacak. Devam edilsin mi?';it = 'Il parametro di calcolo sarà completamente riempito! Continuare?';de = 'Berechnungsparameter werden vollständig neu gefüllt. Fortsetzen?'"), QuestionDialogMode.YesNo, 0);
