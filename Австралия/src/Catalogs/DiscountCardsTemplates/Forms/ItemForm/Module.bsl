...
Procedure CheckTemplate(Command)
	...
	If Not ValueIsFilled(CardCode) Then
		DriveClient.ShowMessageAboutError(ThisForm, "Card code (as an example) is not filled",,,"CardCode");
		Return;
	EndIf;
	...
	If Object.CodeLength > 0 Then
		...
		If CodeLengthAsExample <> Object.CodeLength Then
			DriveClient.ShowMessageAboutError(ThisForm, "Invalid code length. Code length as an example = "+CodeLengthAsExample+".",,,"Object.CodeLength");
			Return;
		EndIf;
	EndIf;
	...
	If Not CodeCorrespondsToMCTemplate() Then
		DriveClient.ShowMessageAboutError(ThisObject, "Invalid prefix, suffix or blocks delimiter");
		Return;
	EndIf;
