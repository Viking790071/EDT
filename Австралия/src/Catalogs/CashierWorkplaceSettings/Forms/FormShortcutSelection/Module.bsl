...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	For Each ItemKey In KeyArray Do
		...
		ShortcutPresentation(NewRow, "Key",            New Shortcut(ItemKey, False,   False,   False));
		
		ShortcutPresentation(NewRow, "Ctrl",           New Shortcut(ItemKey, False,   True, False));
		ShortcutPresentation(NewRow, "Alt",            New Shortcut(ItemKey, True, False,   False));
		ShortcutPresentation(NewRow, "Shift",          New Shortcut(ItemKey, False,   False,   True));
		
		ShortcutPresentation(NewRow, "Alt_Shift",      New Shortcut(ItemKey, True, False,   True));
		ShortcutPresentation(NewRow, "Ctrl_Shift",     New Shortcut(ItemKey, False,   True, True));
		ShortcutPresentation(NewRow, "Ctrl_Alt",       New Shortcut(ItemKey, True, True, False));
		ShortcutPresentation(NewRow, "Ctrl_Alt_Shift", New Shortcut(ItemKey, True, True, True));
...
// i.e. data of products or button using this combination.
// Parameters:
...
//
&AtServer
Procedure ShortcutPresentation(NewRow, Name, Combination)
	...
	Query.Text = 
		...
		|	QuickSale.Products.Description AS ProductsDescription,
		...
		|	NULL
		|FROM
		|	Catalog.CashierWorkplaceSettings.LowerBarButtons AS LowerBarButtons
		|WHERE
	...
	If SelectionDetailRecords.Next() Then
		...
		Else
			NewRow[Name+"_"] = SelectionDetailRecords.ProductsDescription;
		EndIf;
	EndIf;
