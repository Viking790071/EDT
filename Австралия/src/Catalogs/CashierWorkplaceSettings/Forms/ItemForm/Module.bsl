...
// The procedure clears the data of the key combination in the current line of the tabular section transferred through
// the parameter.
//
&AtClient
Procedure ClearShortcut(Shortcut, CurrentData)
	...
	CurrentData.Key = String(Key.None);
...
Procedure FillInButtonsTableFromLayout()
	...
	Template = Catalogs.CashierWorkplaceSettings.GetTemplate("LowerBarButtonsStandardFunctions");
	Template.LanguageCode = CommonClientServer.DefaultLanguageCode();
...
Procedure QuickSaleShortcutStartChoice(Item, ChoiceData, StandardProcessing)
	...
	OpenForm("Catalog.CashierWorkplaceSettings.Form.FormShortcutSelection", 
		New Structure("Workplace", Object.Workplace), 
		ThisObject, 
		UUID,
		,
		,
...
Procedure Fill(Command)
	...
	If Object.LowerBarButtons.Count()>0 Then
		...
		QuestionText = NStr("en = 'Button table is filled in. Clear?'; ru = 'Таблица кнопок заполнена. Очистить?';pl = 'Tabela przycisków jest wypełniona. Wyczyścić?';es_ES = 'Tabla de botones está rellenada. ¿Eliminar?';es_CO = 'Tabla de botones está rellenada. ¿Eliminar?';tr = 'Düğme tablosu dolu. Temizlensin mi?';it = 'La tabella dei pulsanti è compilata. Cancellare?';de = 'Die Schaltflächentabelle ist ausgefüllt. Löschen?'");
...
// Procedure - FillFromProductsGroup command handler of the form.
//
&AtClient
Procedure FillFromProductsGroup(Command)
	
	Notification = New NotifyDescription("FillInFromProductsGroupEnd", ThisForm);
	OpenForm("Catalog.Products.FolderChoiceForm",,,,,,Notification);
	
EndProcedure

// The procedure processes the selection of Products group and fill in QuickSale tabular section by this
// group items.
//
&AtClient
Procedure FillInFromProductsGroupEnd(Result, Parameters) Export
	
	If TypeOf(Result) = Type("CatalogRef.Products") Then
		
		FillByProductsAtServer(Result);
		
	EndIf;
	
EndProcedure

// Procedure fills in QuickSale tabular section by the items of Products group.
//
&AtServer
Procedure FillByProductsAtServer(ProductsGroup)
	...
	Query.Text = 
		...
		|	ProductsCatalog.Ref
		|FROM
		|	Catalog.Products AS ProductsCatalog
		|		LEFT JOIN QuickSale AS QuickSale
		|		ON (QuickSale.Products = ProductsCatalog.Ref)
		|WHERE
		|	QuickSale.Products IS NULL 
		|	AND NOT ProductsCatalog.IsFolder
		|	AND NOT ProductsCatalog.DeletionMark
		|	AND ProductsCatalog.Ref IN HIERARCHY(&ProductsGroup)
		|
		|ORDER BY
		|	ProductsCatalog.Description";
	
	Query.SetParameter("ProductsGroup", ProductsGroup);
	Query.SetParameter("QuickSale", Object.QuickSale.Unload(, "Products"));
	...
	While Selection.Next() Do
		...
		NewRow.Products = Selection.Ref;
	EndDo;
	
	CommonClientServer.MessageToUser(NStr("en = 'Filling in is completed.'; ru = 'Заполнение данных завершено.';pl = 'Wypełnianie jest zakończone.';es_ES = 'Relleno finalizado.';es_CO = 'Relleno finalizado.';tr = 'Doldurma işlemi tamamlandı.';it = 'La compilazione è stata completata.';de = 'Das Ausfüllen ist abgeschlossen.'"));
...
Procedure CopyAllToAnotherWorkplace(Command)
	...
	OpenForm("Catalog.CashierWorkplaceSettings.ChoiceForm",,,,,,Notification);
	
EndProcedure

// The procedure processes the selection of CashierWorkplaceSettings catalog item and fill in QuickSale tabular section
...
// The procedure copies the QuickSale tabular section of a single item of CashierWorkplaceSettings catalog to another one. 
...
Procedure CopySettingsToSourceFromReceiver(Settings, TheReceiver = False)
	...
	If SourceSettings.Ref = ReceiverSettings.Ref Then
		Message = New UserMessage;
		Message.Text = NStr("en = 'Current settings are selected. Copying is not completed.'; ru = 'Выбраны текущие настройки. Копирование не завершено.';pl = 'Bieżące ustawienia są wybrane. Kopiowanie nie jest zakończone.';es_ES = 'Los ajustes actuales están seleccionados. No se ha terminado de copiar.';es_CO = 'Los ajustes actuales están seleccionados. No se ha terminado de copiar.';tr = 'Mevcut ayarlar seçildi. Kopyalama tamamlanmadı.';it = 'Le impostazioni correnti sono selezionate. La copia non è completata.';de = 'Die aktuellen Einstellungen sind ausgewählt. Der Kopiervorgang ist nicht abgeschlossen.'");
		Message.Message();
		Return;
	EndIf;
	
	For Each RowSource In SourceSettings.QuickSale Do
		FromSearch = New Structure("Products, Characteristic", RowSource.Products, RowSource.Characteristic);
	...
	Message = New UserMessage;
	Message.Text = NStr("en = 'Filling in is completed.'; ru = 'Заполнение данных завершено.';pl = 'Wypełnianie jest zakończone.';es_ES = 'Relleno finalizado.';es_CO = 'Relleno finalizado.';tr = 'Doldurma işlemi tamamlandı.';it = 'La compilazione è stata completata.';de = 'Das Ausfüllen ist abgeschlossen.'");
	Message.Message();
