...
Procedure CopyItems(ReceiverValues, ValueSource, CheckEnabled = False, ClearReceiver = True) Export
	...
	For Each ItemSource In SourceElements Do
		...
		Else
			...
			Else
				ItemReceiver = ReceiverElements.Add(Type("DataCompositionFilterItem"));
			EndIf;
		EndIf;
			...
			If StringInventory[PrintForm.ColumnNameCount] > 0 AND StringInventory[PrintForm.ColumnNameTemplate] = PrintForm.Pattern Then
				...
				For Each Draw In Area.Drawings Do
					...
					If Left(Draw.Name, 7) = GetParameterNameBarcode() Then
						...
						BarcodeColumn = ResultStructure.AccordanceFieldsDCSColumnsOfTablesProducts.Get(GetParameterNameBarcode());
						
						If ValueIsFilled(BarcodeColumn) Then
							
							BarcodeValue = StringInventory[BarcodeColumn];
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	SourceData = PrintParameters.PrintInfo[0].Inventory.Unload(New Structure("Selected", True), "Products, Characteristic, Batch, Barcode, PriceTagsQuantity, PriceTagsTemplate, LabelsQuantity, LabelTemplate");
	NeedToPrintTags = PrintManagement.TemplatePrintRequired(PrintFormsCollection, "Price Tags");
	NeedToPrintLabels = PrintManagement.TemplatePrintRequired(PrintFormsCollection, "Labels");
	...
	For Each PrintForm In PrintFormCollectionInner Do
		...
		NewForm.UpperCaseName        = Upper(PrintForm.TemplateName);
	...
	OutputParameters.SendOptions.Insert(
		"Sender",
		?(AvailableAccounts.Count() > 0, AvailableAccounts[0].Ref, Undefined));
	
	OutputParameters.SendOptions.Insert(
		"Subject",
		NStr("en = 'PriceTags and Labels. Formed'; ru = 'Ценники и этикетки. Сформировано.';pl = 'Metki Cenowe i Etykiety. Sformowane';es_ES = 'PriceTags y Etiquetas. Formados';es_CO = 'PriceTags y Etiquetas. Formados';tr = 'PriceTags ve Labels. Oluşturulan';it = 'Cartellini prezzo ed etichette. Create';de = 'PriceTags und Labels. Gebildet'") + " " + CurrentSessionDate());
	
EndProcedure

#EndRegion

#Region LabelAndPriceTAGFormation
...
Function PrepareDataStructure(SettingsStructure) Export
	...
	For Each DataParameter In SettingsStructure.DataParameters Do
		...
		EndIf;
	EndDo;
	SetDCSParameterValue(Composer, "CurrentTime",        CurrentSessionDate());
	...
	FillTabularSectionPricesByDynamicPriceKind(SettingsStructure.DataParameters.Company,
		SettingsStructure.DataParameters.PriceKind,
		ResultStructure.ProductsTable);
	
	Return ResultStructure;
	
EndFunction

Procedure FillTabularSectionPricesByDynamicPriceKind(Company, PriceKind, ProductsTable)
	
	If Not ValueIsFilled(Company) Or Not ValueIsFilled(PriceKind) Then
		Return;
	EndIf;
	
	PriceKindAttributes = Common.ObjectAttributesValues(PriceKind, "PriceCurrency, PriceCalculationMethod");
	
	If PriceKindAttributes.PriceCalculationMethod <> Enums.PriceCalculationMethods.CalculatedDynamic
		And PriceKindAttributes.PriceCalculationMethod <> Enums.PriceCalculationMethods.Formula Then
		Return;
	EndIf;
	
	DataStructure = New Structure;
	InventoryTabularSection = New Array;
	
	DataStructure.Insert("Date",				CurrentSessionDate());
	DataStructure.Insert("Company",				Company);
	DataStructure.Insert("PriceKind",			PriceKind);
	DataStructure.Insert("DocumentCurrency",	PriceKindAttributes.PriceCurrency);
	
	For Each TSRow In ProductsTable Do
		...
		TabularSectionRow.Insert("MeasurementUnit",	Common.ObjectAttributeValue(TSRow.Products, "MeasurementUnit"));
		TabularSectionRow.Insert("Price",			TSRow.Price);
		
		InventoryTabularSection.Add(TabularSectionRow);
		
	EndDo;
	
	DriveServer.GetTabularSectionPricesByPriceKind(DataStructure, InventoryTabularSection);
	
	For Each TSRow In InventoryTabularSection Do
	
		SearchStructure = New Structure;
		SearchStructure.Insert("Products",			TSRow.Products);
		SearchStructure.Insert("Characteristic",	TSRow.Characteristic);
		
		SearchResult = ProductsTable.FindRows(SearchStructure);
