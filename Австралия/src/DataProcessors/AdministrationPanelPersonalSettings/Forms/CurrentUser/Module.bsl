...
Procedure SaveAttributeValue(AttributePathToData, Result)
	...
	If AttributePathToData = "ConfirmationOnApplicationExit" Then
		...
		DriveServerCall.SaveExitConfirmationSettings(ConfirmationOnApplicationExit);
...
Procedure VisibleManagementElements(AttributePathToData = "")
	...
	If AttributePathToData = "FullTextSearch" OR IsBlankString(AttributePathToData) Then
		...
		CommonClientServer.SetFormItemProperty(Items, "ExplanationFullTextSearchInData", "Visible", GetFunctionalOption("UseFullTextSearch"));
		
	EndIf;
	
	If AttributePathToData = "EquipmentConnectionAndSetup" OR IsBlankString(AttributePathToData) Then
		
		CommonClientServer.SetFormItemProperty(Items, "Decoration1", "Visible", GetFunctionalOption("UsePeripherals"));
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	InfobaseUserWithFullAccess = Users.IsFullUser( ,False, False);
	CommonClientServer.SetFormItemProperty(Items, "EquipmentConnectionAndSetup", "Visible", InfobaseUserWithFullAccess);
...
Procedure OnOpen(Cancel)
	...
	ConfirmationOnApplicationExit = StandardSubsystemsClientCached.ClientRunParameters().AskConfirmationOnExit;
