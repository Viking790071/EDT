...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Responsible = DriveReUse.GetValueByDefaultUser(User, "MainResponsible");
	
	DateOfSchedule = CurrentSessionDate();
...
Procedure ScheduleDetailProcessing(Item, Details, StandardProcessing)
	...
	ElsIf RadioButton = "Day" Then
		...
		If Details.Property("DocumentRef")
		   AND TypeOf(Details.DocumentRef) = Type("DocumentRef.EmployeeTask") Then
			OpenForm("Document.EmployeeTask.ObjectForm", New Structure("Key", Details.DocumentRef), , , );
...
Procedure ScheduleEncodingProcessorEnd(Result, AdditionalParameters) Export
	...
	If SelectedType <> Undefined Then
		...
		Else
			...
			OpenForm("Document.EmployeeTask.ObjectForm", FillValue);
		EndIf;
	EndIf;
...
&AtClient
Procedure OpenWeeklyTimesheetJournal(Command)
...
Procedure AddTaskMyCurrentDayTasks(Command)
	...
	OpenForm("Document.EmployeeTask.ObjectForm", New Structure("Employee", Responsible), , , );
...
Procedure ImportFormSettings()
	...
	If TypeOf(SettingsStructure) = Type("Structure") Then
		...
		EndIf;
		If SettingsStructure.Property("EventType") And GetFunctionalOption("UseDocumentEvent") Then
...
Procedure FillPresentationOfPeriod()
	...
	ElsIf RadioButton = "Day" Then
		...
		MonthOfSchedule = Format(DateOfSchedule, "DF=MMM");
		YearOfSchedule = Format(Year(DateOfSchedule), "NG=0");
		WeekDayOfSchedule = DriveClient.GetPresentationOfWeekDay(DateOfSchedule);
...
Procedure RepresentMyAgenda()
	...
	|		CASE
	|			WHEN WorkOrderWorks.Ref.State = VALUE(Catalog.JobAndEventStatuses.Completed)
	|				THEN TRUE
	|			ELSE FALSE
	|		END
	|	FROM
	...
	Query.SetParameter("UseDocumentEvent", UseDocumentEvent);
...
Procedure RepresentScheduleDay()
	...
	|		WHERE
	|			WorkOrderWorks.Ref.Posted = TRUE
	|			AND WorkOrderWorks.Ref.Employee IN(&EmployeesList)
	|			AND DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.BeginTime) * 3600 + MINUTE(WorkOrderWorks.BeginTime) * 60 + SECOND(WorkOrderWorks.BeginTime)) >= &BeginTime
	|			AND DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.EndTime) * 3600 + MINUTE(WorkOrderWorks.EndTime) * 60 + SECOND(WorkOrderWorks.EndTime)) <= &EndTime
	|			AND (&Counterparty = VALUE(Catalog.Counterparties.EmptyRef)
	|					OR WorkOrderWorks.Customer = &Counterparty)
	...
	IterationByResponsible = Query.Execute().Select(QueryResultIteration.ByGroups);
...
Procedure RepresentScheduleMonth()
	...
	|			END,
	|			CASE
	|				WHEN NOT WorkOrderWorks.Ref.State = VALUE(Catalog.JobAndEventStatuses.Completed)
	|						AND DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.EndTime) * 3600 + MINUTE(WorkOrderWorks.EndTime) * 60 + SECOND(WorkOrderWorks.EndTime)) > &DateOfSchedule
	|					THEN 1
	|				ELSE 0
	|			END,
	|			2,
	...
	|			PRESENTATION(WorkOrderWorks.Customer),
	|			WorkOrderWorks.Day,
	|			DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.BeginTime) * 3600 + MINUTE(WorkOrderWorks.EndTime) * 60 + SECOND(WorkOrderWorks.EndTime)),
	|			DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.EndTime) * 3600 + MINUTE(WorkOrderWorks.EndTime) * 60 + SECOND(WorkOrderWorks.EndTime)),
	|			CASE
	|				WHEN WorkOrderWorks.Ref.State = VALUE(Catalog.JobAndEventStatuses.Completed)
	|					THEN TRUE
	...
	|		WHERE
	|			WorkOrderWorks.Ref.Posted = TRUE
	|			AND DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.BeginTime) * 3600 + MINUTE(WorkOrderWorks.EndTime) * 60 + SECOND(WorkOrderWorks.EndTime)) >= &BeginOfPeriod
	|			AND DATEADD(WorkOrderWorks.Day, SECOND, HOUR(WorkOrderWorks.EndTime) * 3600 + MINUTE(WorkOrderWorks.EndTime) * 60 + SECOND(WorkOrderWorks.EndTime)) <= &EndOfPeriod
	|			AND WorkOrderWorks.Ref.Employee IN(&EmployeesList)
	|			AND (&Counterparty = VALUE(Catalog.Counterparties.EmptyRef)
	|					OR WorkOrderWorks.Customer = &Counterparty)
	|			AND (&State = VALUE(Catalog.JobAndEventStatuses.EmptyRef)
	|					OR WorkOrderWorks.Ref.State = &State)
	|			AND (&EventType = VALUE(Enum.EventTypes.EmptyRef)
		...
		For CurWeek = 1 To 6 Do
			...
			For CurWeekday = 1 To 7 Do
				...
				If BegOfMonth(CurDate) <> BegOfMonth(DateOfSchedule) Then
					Schedule.Area(DayCoordinates).TextColor = StyleColors.TextLeaveEmpty;
				EndIf;
					...
					If BegOfDay(CurDate) >= BegOfDay(SelectionOfQueryResult.BeginTime)
					   AND BegOfDay(CurDate) <= BegOfDay(SelectionOfQueryResult.BeginTime) Then
						...
						ElsIf CtEventsAndJobs <= 6 Then
							...
							ElsIf ValueIsFilled(SelectionOfQueryResult.ProductsPresentation) Then
								Subject = SelectionOfQueryResult.ProductsPresentation;
							ElsIf ValueIsFilled(SelectionOfQueryResult.Comment) Then
...
Procedure MarkCompletedJobAtServer(DocumentRef, Closed)
	...
	If DocumentObject.State = Catalogs.JobAndEventStatuses.Completed Then
		DocumentObject.State = Catalogs.JobAndEventStatuses.Planned;
	Else
		DocumentObject.State = Catalogs.JobAndEventStatuses.Completed;
	EndIf;
