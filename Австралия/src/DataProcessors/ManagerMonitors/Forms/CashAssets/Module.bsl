...
Procedure GetSettings()
	...
	Interval = Common.CommonSettingsStorageLoad("SettingsForMonitors", "Interval", Enums.Periodicity.Day);
...
Procedure SaveSettings()
	...
	Common.CommonSettingsStorageSave("SettingsForMonitors", "Interval", Interval);
...
Procedure RefreshChartCashAssetsBalance()
	...
	Query.Text = StrReplace(Query.Text, "Day", GetPeriodicity(Interval));
...
Procedure RefreshCAReceiptAndExpenseChart()
	...
	While Selection.Next() Do
		...
		ToolTip = "Receipt " + Selection.AmountReceipt + " on " + Format(Selection.Period, "DLF=D");
	...
	While Selection.Next() Do
		...
		Point.Text = Format(Selection.Period, "DLF=D");
		ToolTip = "Expense " + Selection.AmountExpense + " on " + Format(Selection.Period, "DLF=D");
...
Procedure RefreshWidgetCashAssetsBalance()
	...
	Query.Text =
		...
		|			&Period,
		|			Company = &Company
		|				AND PaymentMethod.CashAssetType = VALUE(Enum.CashAssetTypes.Noncash)) AS CashAssetsBalances
		|
		|ORDER BY
		|	AmountBalance DESC
		|TOTALS
		...
		|			&Period,
		|			Company = &Company
		|				AND PaymentMethod.CashAssetType = VALUE(Enum.CashAssetTypes.Cash)) AS CashAssetsBalances
		|
		|ORDER BY
		|	AmountBalance DESC
		|TOTALS
	...
	If SelectionTotals.Next() Then
		Items.DecorationBankAccountsTotal.Title = DriveServer.GenerateTitle(SelectionTotals.AmountBalance);
	Else
	...
	If SelectionTotals.Next() Then
		Items.DecorationCashAccountsTotal.Title = DriveServer.GenerateTitle(SelectionTotals.AmountBalance);
	Else
		Items.DecorationCashAccountsTotal.Title = "—";
	EndIf;
...
Procedure RefreshProfitWidget()
	...
	Query.Text =
		"SELECT ALLOWED
		|	CASE
		|		WHEN CashFundsBalanceAndTurnovers.Recorder REFS Document.OpeningBalanceEntry
	...
	If SelectionTotals.Next() Then
		Items.AmountIncomeTotal.Title = DriveServer.GenerateTitle(SelectionTotals.Amount);
	Else
...
Procedure RefreshWidgetLoss()
	...
	SelectionTotals = Query.Execute().Select(QueryResultIteration.ByGroups);
	If SelectionTotals.Next() Then
		Items.AmountExpensesTotal.Title = DriveServer.GenerateTitle(SelectionTotals.Amount);
	Else
...
Function GetPeriodicity(Value)
	...
	If Value = Enums.Periodicity.Day Then
		Return "Day";
	ElsIf Value = Enums.Periodicity.Week Then
		Return "Week";
	ElsIf Value = Enums.Periodicity.Month Then
		Return "Month";
	ElsIf Value = Enums.Periodicity.Quarter Then
		Return "Quarter";
	EndIf;
